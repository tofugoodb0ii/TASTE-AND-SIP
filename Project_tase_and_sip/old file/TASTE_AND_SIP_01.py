...
# (แก้: อย่า join กับพาธเต็ม จะเพี้ยน)
...
class CategoryPage(ctk.CTkFrame):
    ...
    def __init__(self, app: CTkMain):
        ...
        self.gridf.bind("<Configure>", lambda e: self.render())  # รีเรนเดอร์ตอนขนาดเปลี่ยน
    ...
    def render(self):
        ...
        card_w, card_h = 300, 220              # ปรับขนาดการ์ดตามที่ต้องการ
        gap = 20
        area_w = max(1, self.gridf.winfo_width())
        cols = max(1, (area_w // (card_w + gap)))   # ได้ 3,4,5... ตามความกว้างจริง
        # ตั้ง weight ให้ทุกคอลัมน์
        for i in range(cols):
            self.gridf.grid_columnconfigure(i, weight=1)
        ...
        for p in products:
            ...
            card.grid(row=row, column=col, padx=10, pady=10, sticky="n")  # หรือ "nwe" ถ้าอยากยืด
            ...
            ctk.CTkLabel(card, text=f"{float(p['base_price']):,.2f} ฿",   # ใส่คอมม่าให้ราคาด้วย
...
class AdminReports(ctk.CTkFrame):
    ...
    def run_prod(self):
        def work():
            s,e=self._range()
            data=self.db.report_by_product(s,e)
            return [(r['product'], r['qty'], f"{(r['sales'] or 0):,.2f}") for r in data]
        self._safe_run(work, ["PRODUCT","QTY","SALES (฿)"])
