# -*- coding: utf-8 -*-
# TASTE AND SIP — Cart/Payment in dialogs + Save Bill + Slip upload
# เน้นฝั่งลูกค้า: ไม่มี Customize/Size/Sweetness, ไม่มี Channel/Pickup
# ปุ่มบนแถบ: Cart, Checkout & Payment
# VAT 7%, Receipt PDF เมื่อจ่าย, Save Bill PDF ได้จากตะกร้า
# Admin: ไม่มี Inventory/Gross Margin (ซ่อนจาก UI)
...
IMG_QR_PATH         = os.path.join(IMG_DIR, r"C:\Users\thatt\Downloads\qrcode.jpg")   # ใส่ QR ร้านไว้
...
class DB:
    ...
    def _schema(self):
        ...
        # (คงตาราง inventory/bom/stock_movements เดิมไว้ แต่ UI ไม่ใช้)
    ...
    def create_order(self, user_id:int, cart_items:List[Dict], promo_code:str, payment_method="SLIP", payment_ref=""):
        ...
        self.conn.commit()
        # (ถ้ามี BOM จะตัดสต๊อก; UI ไม่แสดงเรื่อง Inventory)
...
class CartDialog(tk.Toplevel):
    ...
    def apply_code(self):
        self.app.cart_promo = self.var_code.get().strip().upper()
        self.refresh()
...
class PaymentDialog(tk.Toplevel):
    def __init__(self, app):
        super().__init__(app); self.app=app
        self.title("Checkout & Payment"); self.geometry("560x560"); self.resizable(False, False)
        wrap=ttk.Frame(self,padding=10); wrap.pack(fill="both",expand=True)

        # QR ร้าน
        ttk.Label(wrap,text="Payment: QR").pack(anchor="w")
        self.qr_canvas=tk.Canvas(wrap,width=220,height=220,bg="#fff",highlightthickness=1,highlightbackground="#ddd")
        self.qr_canvas.pack(pady=(4,10))
        self._qr=None; self._draw_qr()

        # สลิป
        ttk.Label(wrap,text="Customer Slip").pack(anchor="w")
        self.slip_canvas=tk.Canvas(wrap,width=220,height=220,bg="#fff",highlightthickness=1,highlightbackground="#ddd")
        self.slip_canvas.pack(pady=(4,6))
        ttk.Button(wrap,text="Upload Slip...",command=self.upload).pack()
        self._slip=None; self.slip_path=self.app.slip_path or None
        if self.slip_path: self._show_slip(self.slip_path)

        ttk.Separator(wrap).pack(fill="x",pady=8)

        # totals
        s,d,v,t = self._calc_totals()
        self.lbl_totals=ttk.Label(wrap, text=self._totals_text(s,d,v,t), justify="right")
        self.lbl_totals.pack(fill="x")

        ttk.Button(wrap,text="Confirm & Checkout",command=self.checkout).pack(pady=8, fill="x")
    ...
    def _draw_qr(self):
        ...
        if os.path.exists(IMG_QR_PATH):
            try:
                img=Image.open(IMG_QR_PATH).convert("RGB"); img.thumbnail((220,220), Image.LANCZOS)
                self._qr=ImageTk.PhotoImage(img); self.qr_canvas.create_image(110,110,image=self._qr); return
            except: pass
        self.qr_canvas.create_text(110,110,text="QR Placeholder\nassets/images/qr.png",justify="center")

    def _show_slip(self, path):
        try:
            img=Image.open(path).convert("RGB"); img.thumbnail((220,220), Image.LANCZOS)
            self._slip=ImageTk.PhotoImage(img); self.slip_canvas.delete("all"); self.slip_canvas.create_image(110,110,image=self._slip)
        except:
            self.slip_canvas.delete("all"); self.slip_canvas.create_text(110,110,text="preview failed")
    ...
    def checkout(self):
        ...
        oid, s, d, v, t = self.app.db.create_order(
            ...
            promo_code=self.app.cart_promo or "",
            payment_method="SLIP", payment_ref=self.slip_path
        )
...
class App(tk.Tk):
    ...
    def show(self,name):
        target={"Login":"LoginView","Register":"RegisterView","Shop":"ShopView",
                "Orders":"OrdersView","Profile":"ProfileView","Admin":"AdminView"}[name]
...
# ---- Views (Login/Register/Shop/Orders/Profile/Admin) ----
...
# ======================= Auth (customtkinter) =======================
RIGHT_BG="#f8eedb"; CARD_BG="#edd8b8"; TEXT_DARK="#1f2937"; LINK_FG="#0057b7"; BORDER="#d3c6b4"; CARD_W=660; CARD_H=560; RADIUS=18
...
class AuthApp(ctk.CTk):
    def __init__(self, db_path=DB_FILE, left_bg_path=None, logo_path=None, on_login_success=None):
        ...
        self.left_canvas=tk.Canvas(self.left, highlightthickness=0, bd=0, bg=RIGHT_BG); self.left_canvas.place(x=0,y=0,relwidth=1,relheight=1)
        self._left_img_tk=None; self.left.bind("<Configure>", lambda e:self._draw_left_bg())
        ...
        self.logo_wrap=ctk.CTkFrame(self.right, fg_color=RIGHT_BG); self.logo_wrap.grid(row=0,column=0,pady=(30,10)); self._render_logo()
    ...
    def _draw_left_bg(self):
        ...
        w=max(300,int(self.winfo_width()*0.5)); h=max(300,self.winfo_height()); c.configure(width=w,height=h)
        c.create_rectangle(0,0,w,h,fill=RIGHT_BG,outline="")
        if self.left_bg_path and os.path.exists(self.left_bg_path):
            try:
                img=Image.open(self.left_bg_path).convert("RGB"); iw,ih=img.size
                scale=max(w/iw,h/ih); nw,nh=int(iw*scale),int(ih*scale); img=img.resize((nw,nh),Image.LANCZOS)
                left=max(0,(nw-w)//2); top=max(0,(nh-h)//2); img=img.crop((left,top,left+w,top+h))
                self._left_img_tk=ImageTk.PhotoImage(img); c.create_image(0,0,anchor="nw",image=self._left_img_tk)
            except: pass
    ...
    def _render_logo(self):
        ...
        if self.logo_path and os.path.exists(self.logo_path):
            try:
                img=Image.open(self.logo_path); self._logo_img=ctk.CTkImage(light_image=img,dark_image=img,size=(220,220))
    ...
    def show_forgot(self):
        ...
        self.fp_step2=ctk.CTkFrame(self.card,fg_color="transparent"); self.fp_step2.pack(fill="x",padx=16,pady=(6,12)); self.fp_step2.pack_forget()
    ...
    def _forgot_verify(self):
        ...
        if row: self._verified_username=u; self.fp_err.set("VERIFIED. PLEASE SET A NEW PASSWORD BELOW."); self.fp_step2.pack(fill="x",padx=16,pady=(6,12))
        else: self.fp_err.set("NO MATCHING ACCOUNT FOR THE GIVEN USERNAME AND EMAIL/PHONE.")
