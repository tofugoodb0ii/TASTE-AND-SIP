# -*- coding: utf-8 -*-
# TASTE AND SIP (Customer-simple + QR separate + no Inventory/GrossMargin)
# - ลูกค้า: ไม่มี Customize/Size/Sweetness, ไม่มี Channel/Pickup
# - เพิ่มกรอบ "QR ร้าน" แยกจาก "สลิปลูกค้า" ชัดเจน
# - สรุปราคา: VAT 7% ท้ายบิล
# - Checkout ต้องอัปโหลดสลิปก่อน, ใบเสร็จ PDF แสดง VAT
# - ฝั่งแอดมิน: ตัดแท็บ Inventory ออก, ตัดปุ่มรายงาน Gross Margin ออก (ซ่อนจาก UI)
# - DB: คงตารางเดิมไว้ทั้งหมด, เพิ่มคอลัมน์ orders.vat ถ้ายังไม่มี
...
IMG_QR_PATH         = os.path.join(IMG_DIR, "qr.png")  # <- ใส่รูป QR ร้านไว้ที่นี่
...
class DB:
    ...
    # --- inventory / BOM (คงไว้สำหรับตัดสต๊อกจาก BOM ถ้ามีข้อมูล) ---
    def bom(self, pid):
    ...
    # --- admin CRUD/promos/reports (ซ่อน Inventory & Gross Margin ออกไปจาก UI) ---
...
def create_receipt_pdf(order_id, db: DB, user_row):
    ...
    canv.setFont("Helvetica", 10)
    canv.drawString(x, y, f"Payment: {order['payment_method']} (slip saved in system)")
...
class ShopView(ttk.Frame):
    def __init__(self, master, app: App):
        ...
        self.qr_canvas = tk.Canvas(right,width=200,height=200,bg="#fff",highlightthickness=1,highlightbackground="#ddd")
        self.qr_canvas.pack(pady=(4,2))
        self._qr_imgtk=None
        self._draw_qr()

        # --- สลิปลูกค้า (กรอบล่าง) ---
        ttk.Label(right,text="Customer Slip").pack(anchor="w")
        self.slip_canvas = tk.Canvas(right,width=200,height=200,bg="#fff",highlightthickness=1,highlightbackground="#ddd")
        self.slip_canvas.pack(pady=(4,2))
        self._slip_imgtk=None; self.slip_path=None
        ttk.Button(right,text="Upload Slip...",command=self.upload_slip).pack(fill="x",pady=(0,6))
    ...
    def on_show(self):
        self._build_tabs(); self.refresh(); self._draw_qr()

    def _draw_qr(self):
        self.qr_canvas.delete("all")
        if os.path.exists(IMG_QR_PATH):
            try:
                img=Image.open(IMG_QR_PATH).convert("RGB")
                ...
                pass
        self.qr_canvas.create_text(100,100,text="QR Placeholder\nassets/images/qr.png",justify="center")
...
class AdminView(ttk.Frame):
    def __init__(self, master, app: App):
        ...
        # แสดงเฉพาะ Products / Promotions / Reports (ไม่มี Inventory)
    ...
    # --- Reports tab (ไม่มี Gross Margin) ---
    def _reports(self):
