...
class ShopView(ttk.Frame):
    def __init__(self, master, app: App):
        ...
        # --- ปุ่มยืนยันออเดอร์ (Confirm) ใต้ QR และสลิป ---
        btn_row = ttk.Frame(pay)
        btn_row.grid(row=1, column=0, columnspan=2, sticky="e", pady=(8, 0))
        self.btn_confirm = tk.Button(
            btn_row,
            text="ยืนยันออเดอร์ (Confirm)",
            bg="#28a745", fg="white",
        ...
        # ปุ่ม Checkout สีชัด อ่านง่าย (คงไว้ตามเดิม)
...
class AdminView(ttk.Frame):
    ...
    def reload_promos(self):
        # NOTE: ฟังก์ชัน list_promotions() / delete_promotion() ต้องมีใน DB ต้นฉบับของคุณ
