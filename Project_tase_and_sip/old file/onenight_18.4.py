...
# - ปรับ Forgot Password ให้เป็น 2 คอลัมน์ (Verify -> ตั้งรหัสใหม่) และไม่ล้นกรอบ
# - เพิ่มช่องค้นหาในหน้า Shop
# - ปุ่ม Checkout สีชัด อ่านง่าย
...
def create_receipt_pdf(order_id, db: DB, user_row):
    """
    จัดรูปแบบใบเสร็จให้เหมือนภาพตัวอย่าง:
    โลโก้/เบอร์/TAX/DATE/TIME/ORDER NO + ตาราง No., MENU, QTY., PRICE, TOTAL + VAT + TOTAL
    """
    ensure_dirs()
...
class ShopView(ttk.Frame):
    def __init__(self, master, app: App):
        ...
        self.fab = tk.Frame(self, bg="", highlightthickness=0)
        self.fab.place(relx=1.0, rely=1.0, x=-18, y=-18, anchor="se")

        self.btn_confirm = tk.Button(
            self.fab, text="CONFIRM",
            bg="#10b981", fg="white",
            activebackground="#0aa06f", activeforeground="white",
            font=("Segoe UI", 10, "bold"), padx=14, pady=8, relief="raised",
            command=self._confirm_before_checkout
        )
        self.btn_confirm.pack(side="left", padx=(0, 8))

        self.btn_checkout_fab = tk.Button(
            self.fab, text="CHECKOUT",
            bg="#2c7be5", fg="white",
            activebackground="#1f6fdd", activeforeground="white",
            font=("Segoe UI", 10, "bold"), padx=14, pady=8, relief="raised",
            command=self.checkout
        )
        self.btn_checkout_fab.pack(side="left")
    ...
    def clear(self):
        ...
        self.btn_confirm = tk.Button(
            self.fab, text="CONFIRM",
            bg="#10b981", fg="white", activebackground="#0aa06f", activeforeground="white",
            font=("Segoe UI", 10, "bold"), padx=14, pady=8, relief="raised",
            command=self._confirm_before_checkout
        )
        self.btn_confirm.pack(side="left", padx=(0,8))
        ...
        def _confirm_before_checkout(self):
            ...
            if messagebox.askyesno("Confirm", "Confirm this order and proceed to checkout?"):
                self.checkout()
...
class AdminView(ttk.Frame):
    ...
    # --- Reports tab (ตัด Gross Margin ออก) ---
    def _reports(self):
...
class AuthApp(ctk.CTk):
    def __init__(self, db_path: str = DB_FILE, left_bg_path: Optional[str] = None,
        ...
        self.card.grid_propagate(False)          # คงขนาดกรอบทุกหน้า
    ...
    def show_forgot(self):
        ...
        # แถวที่ 1: 2 คอลัมน์ (USERNAME | EMAIL/PHONE)
        ...
        # ขั้นตอน 2: ตั้งรหัสใหม่ (แสดงหลัง verify) — 2 คอลัมน์
