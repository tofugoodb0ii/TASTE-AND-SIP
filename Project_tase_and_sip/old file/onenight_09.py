# -*- coding: utf-8 -*-
"""
TASTE & SIP — Auth + Admin (customtkinter)
- Sign Up 2 คอลัมน์ พร้อมขนาดช่องเท่ากัน
- ช่องกรอกทุกหน้ามีความสูงเท่ากัน (FIELD_H) และธีมเดียวกัน
- ซ้าย: Canvas ภาพเต็ม + ตัวหนังสือสีขาว
- ขวา: การ์ดโทนครีมโค้งมน
...
# ===== Size tokens =====
FIELD_W        = 380   # ความกว้างมาตรฐาน (คอลัมน์เดียว)
FIELD_W_HALF   = 240   # ความกว้างช่องครึ่งหนึ่ง ใช้ในหน้า Sign Up (2 คอลัมน์)
FIELD_W_DOUBLE = FIELD_W_HALF*2 + 20  # ความกว้างครอบสองคอลัมน์ (อีเมล)
FIELD_H        = 42    # ความสูงช่อง
FORM_PADX      = 24    # ระยะขอบซ้าย/ขวา
...
class LabeledEntry(ctk.CTkFrame):
    """ช่องกรอกพร้อมป้าย — รองรับกำหนด width เพื่อใช้ได้ทั้งคอลัมน์เดี่ยว/คู่"""
...
class AuthApp(ctk.CTk):
    ...
    def show_signin(self):
        ...
        self.si_err = ErrorLabel(self.card); self.si_err.pack(padx=FORM_PADX, fill="x")
    ...
    def show_signup(self):
        ...
        self.su_err = ErrorLabel(self.card); self.su_err.pack(padx=FORM_PADX, fill="x")
    ...
    def show_forgot(self):
        ...
        self.fp_err = ErrorLabel(self.card); self.fp_err.pack(padx=FORM_PADX, fill="x")
...
class AdminApp(ctk.CTk):
    ...
    def _demo_order(self):
        try:
            p = self.db.conn.execute("SELECT id, base_price, name FROM products ORDER BY RANDOM() LIMIT 1").fetchone()
            if p:
                cur = self.db.conn.cursor()
                subtotal = float(p["base_price"] or 0)
                ...
                cur.execute("""INSERT INTO orders(user_id, order_datetime, status, subtotal, discount, vat, total, note)
                               VALUES(?,?,?,?,?,?,?,?)""",
                            (None, now_ts(), "PAID", subtotal, 0.0, vat, total, "demo"))
                oid = cur.lastrowid
                ...
                self.db.conn.commit()
                messagebox.showinfo("Demo", f"Created demo order #{oid}")
        except Exception as e:
