# -*- coding: utf-8 -*-
# TASTE AND SIP - Full Single-File App (Tkinter + SQLite + Pillow + ReportLab)
# Customer + Admin, Profiles, Catalog with Size/Sweetness/Toppings, Cart + Promotions,
# QR Payment (image), PDF Receipt, Inventory with BOM & Low-stock alerts, Sales Reports.
...
IMG_QR_PATH         = os.path.join(IMG_DIR, "qr.png")       # <- ใส่รูปเองภายหลัง
...
class DB:
    ...
    def _schema(self):
        ...
        CREATE TABLE IF NOT EXISTS promotions(
            ...
            applies_to_product_id INTEGER,   -- NULL = whole-bill
    ...
    def _seed(self):
        ...
        if c.execute("SELECT COUNT(*) n FROM promotions").fetchone()['n'] == 0:
            ...
            # Welcome 10% whole bill + 5 ฿ off Thai Milk Tea per item
    ...
    def create_order(self, user_id, channel, pickup_time, cart_items, promo_code, payment_method="QR"):
        # compute subtotal
        subtotal = 0.0
        item_totals = []  # for item-level promos
        ...
        if promo:
            ...
            if ptype in ("PERCENT_BILL","FLAT_BILL") and subtotal >= msp:
                ...
                # apply to matching product_id items only
    ...
    for it in items:
        ...
        for tp in tps:
            if tp['order_item_id']==it['id']:
                ...
                total_tops += tp['price']*tp['qty']; y -= 5*mm
        if it['note']:
...
class ProfileView(ttk.Frame):
    ...
    def change_avatar(self):
        ...
        try:
            ...
            self.app.user = self.app.db.auth(self.app.user['username'], self.app.user['username']+"?") or self.app.db.conn.execute("SELECT * FROM users WHERE id=?", (self.app.user['id'],)).fetchone()
