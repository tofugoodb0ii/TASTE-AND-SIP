...
# - ปุ่ม CONFIRM + CHECKOUT & CONFIRM มุมขวาล่าง (ต้องอัปโหลดสลิปก่อนกดได้)
# - ใบเสร็จ PDF ตามเลย์เอาต์ตัวอย่าง + เปิดอัตโนมัติหลังสั่งซื้อ
# - Forgot Password 2 ขั้น ไม่ล้นกรอบ
# - ช่องค้นหาในหน้า Shop
# - ปุ่ม Download Latest Receipt สำหรับเปิดใบเสร็จออเดอร์ล่าสุดได้ตลอด
...
class DB:
    ...
    def _seed(self):
        ...
        if c.execute("SELECT COUNT(*) n FROM promotions").fetchone()['n'] == 0:
            ...
            c.execute("""INSERT INTO promotions(code,type,value,min_spend,start_at,end_at,applies_to_product_id,is_active)
                         VALUES(?,?,?,?,?,?,?,?)""", ("WELCOME10","PERCENT_BILL",10,0,st,ed,None,1))
        self.conn.commit()
    ...
    # promotions
    def find_promo(self, code): return self.conn.execute("SELECT * FROM promotions WHERE code=? AND is_active=1",(code,)).fetchone()
    def list_promotions(self): return self.conn.execute("SELECT * FROM promotions ORDER BY id DESC").fetchall()
    def delete_promotion(self, pid:int): self.conn.execute("DELETE FROM promotions WHERE id=?", (pid,)); self.conn.commit()
    ...
    def latest_order_id(self, uid) -> Optional[int]:
        r = self.conn.execute("SELECT id FROM orders WHERE user_id=? ORDER BY id DESC LIMIT 1", (uid,)).fetchone()
...
def create_receipt_pdf(order_id, db: DB, user_row):
    ...
    for i, it in enumerate(items, 1):
        ...
        for val, w in zip(row, widths): c.drawString(x+2, y, val); x += w
        y -= 7*mm
...
class ShopView(ttk.Frame):
    def __init__(self, master, app: App):
        ...
        right.pack_propagate(False); right.configure(width=360)
        ...
        pay.grid_columnconfigure(0, weight=1); pay.grid_columnconfigure(1, weight=1)
        ...
        if os.path.exists(IMG_QR_PATH):
            try:
                qimg = Image.open(IMG_QR_PATH).convert("RGB"); qimg.thumbnail((200,200), Image.LANCZOS)
                self._qr_imgtk = ImageTk.PhotoImage(qimg); self.qr_canvas.create_image(100,100,image=self._qr_imgtk)
            except Exception:
        ...
        self.btn_receipt = ttk.Button(right, text="Download Latest Receipt", command=self.download_latest_receipt)
        self.btn_receipt.pack(fill="x", pady=(4,8))
        ...
        self.btn_checkout = tk.Button(
            bottom_bar, text="CHECKOUT & CONFIRM", command=self.checkout,
    ...
    def _update_action_state(self):
        enabled = bool(self.app.cart) and bool(self.slip_path)
        state = tk.NORMAL if enabled else tk.DISABLED
        self.btn_confirm.configure(state=state)
        self.btn_checkout.configure(state=state)

    def on_show(self):
        self._build_tabs(); self.refresh(); self._update_action_state()
    ...
    def _build_tabs(self):
        ...
        for cat in self.app.db.categories():
            ...
            if q: prods = [p for p in prods if (p['name'] or "").lower().find(q) != -1]
    ...
    def add(self, prod_row):
        ...
        self.refresh(); self._update_action_state()
    ...
    def upload_slip(self):
        ...
        self.slip_path=f
        try:
            img=Image.open(f).convert("RGB"); img.thumbnail((200,200), Image.LANCZOS)
            self._slip_imgtk=ImageTk.PhotoImage(img)
            self.slip_canvas.delete("all"); self.slip_canvas.create_image(100,100,image=self._slip_imgtk)
        except Exception:
            self.slip_canvas.delete("all"); self.slip_canvas.create_text(100,100,text="(preview failed)")
        self._update_action_state()
    ...
    def checkout(self):
        ...
        self.app.cart.clear(); self.slip_path=None; self.slip_canvas.delete("all")
        self.refresh(); self._update_action_state(); self.app.show("Orders")

    def download_latest_receipt(self):
        if not self.app.user:
            messagebox.showinfo("Receipt", "Please sign in first."); return
        oid = self.app.db.latest_order_id(self.app.user['id'])
        if not oid:
            messagebox.showinfo("Receipt", "No orders yet."); return
        path = create_receipt_pdf(oid, self.app.db, self.app.user)
        try:
            if sys.platform.startswith("win"): os.startfile(path)
            elif sys.platform=="darwin": os.system(f"open '{path}'")
...
class OrdersView(ttk.Frame):
    def __init__(self, master, app: App):
        ...
        self.tv.bind("<Double-1>", lambda e: self.open())
...
# ---------- Admin (promotions & basic products) ----------
...
class PromoEditor(tk.Toplevel):
    def __init__(self, master, db: DB, pid, on_done):
        ...
        frm=ttk.Frame(self,padding=10); frm.pack(fill="both",expand=True)
        fields=[("Code","code"),("Type [PERCENT_BILL/FLAT_BILL]","type"),
                ("Value","value"),("Min Spend","min"),("Start (YYYY-MM-DD HH:MM:SS)","start"),
                ("End","end"),("Active 1/0","act")]
        self.inp={}
        for i,(label,key) in enumerate(fields):
            ttk.Label(frm,text=label).grid(row=i,column=0,sticky="e")
            e=ttk.Entry(frm,width=38); e.grid(row=i,column=1,pady=3); self.inp[key]=e
...
def _launch_auth():
    # สำหรับเดโม ใช้ ttk login ด้านบน
    app = App()
    app.mainloop()
