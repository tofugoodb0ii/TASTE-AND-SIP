...
IMG_QR_PATH         = os.path.join(IMG_DIR, "qr.png")       # <- วางรูปคิวอาร์เอง (ไม่มีก็ขึ้น placeholder)
...
def valid_username(u: str) -> bool:
    # อนุญาต a-z A-Z 0-9 _ . และยาว 3-30
    return bool(re.fullmatch(r"[A-Za-z0-9_.]{3,30}", u))

def valid_password(p: str) -> bool:
    # ยาว >= 6 พอ (ผ่อนคลาย) — ไม่ว่างเปล่า
    return len(p) >= 6

def valid_email(e: str) -> bool:
    if not e: return True  # optional
    return bool(re.fullmatch(r"[^@\s]+@[^@\s]+\.[^@\s]+", e))

def valid_phone(ph: str) -> bool:
    if not ph: return True  # optional
    return bool(re.fullmatch(r"[0-9+\- ]{6,20}", ph))
...
class DB:
    ...
    def _seed(self):
        ...
        if not c.execute("SELECT 1 FROM users WHERE username='admin'").fetchone():
            c.execute(
                ...
                ("admin", sha256("admin123"), "Administrator", "admin", "0800000000", "admin@example.com")
            )
        # categories
    ...
    # --- user/auth/profile ---
    def create_user(self, username, password, phone=None, email=None, name=None):
        try:
            self.conn.execute(
                "INSERT INTO users(username,password_hash,name,phone,email,role) VALUES(?,?,?,?,?,?)",
                (username, sha256(password), name or "", phone or "", email or "", "customer")
            )
...
class ProductCard(ttk.Frame):
    """การ์ดสินค้า: แยกปุ่ม Customize และ Add (เพิ่มไว)"""
    def __init__(self, master, row, on_customize, on_quick_add):
        super().__init__(master, padding=6, style="Card.TFrame")
        self.row = row; self.on_customize = on_customize; self.on_quick_add = on_quick_add
        ...
        ttk.Button(btns, text="Customize", command=lambda: self.on_customize(row)).grid(row=0, column=0, padx=2)
        ttk.Button(btns, text="Add", command=lambda: self.on_quick_add(row)).grid(row=0, column=1, padx=2)
...
class App(tk.Tk):
    ...
    def logout(self):
        # ไม่ปิดโปรแกรม — รีเซ็ตสถานะและย้อนกลับหน้า Login
...
class RegisterView(ttk.Frame):
    def __init__(self, master, app: App):
        ...
        self.ename=self._row(frm,"Name",3)
        self.ephone=self._row(frm,"Phone",4)
        self.eemail=self._row(frm,"Email",5)
    ...
    def reg(self):
        ...
        nm=self.ename.get().strip(); ph=self.ephone.get().strip(); em=self.eemail.get().strip()
        if not u or not p1 or not p2: messagebox.showerror("Error","Fill username/password"); return
        if p1!=p2: messagebox.showerror("Error","Passwords do not match"); return
        if not valid_username(u): messagebox.showerror("Error","Invalid username"); return
        if not valid_password(p1): messagebox.showerror("Error","Password must be at least 6 chars"); return
        if not valid_email(em): messagebox.showerror("Error","Invalid email"); return
        if not valid_phone(ph): messagebox.showerror("Error","Invalid phone"); return
        ok,msg=self.app.db.create_user(u,p1,phone=ph,email=em,name=nm)
    ...
    def _build_tabs(self):
        ...
        for cat in self.app.db.categories():
            ...
            for i,p in enumerate(self.app.db.products_by_cat(cat['id'])):
                ...
                # ใส่ปุ่ม Customize / Add
                ProductCard(rowf,p,self.customize,self.quick_add).grid(row=0,column=col,padx=6); col+=1

    def customize(self, prod_row):
        ProductDialog(self, self.app.db, prod_row, self.app.db.toppings(), self.add)

    def quick_add(self, prod_row):
        """เพิ่มแบบเร็ว: size=S, sweetness=50%, no toppings, qty=1"""
        item = {
            "product_id": prod_row['id'], "name": prod_row['name'],
            "base_price": float(prod_row['base_price']),
            "size": "S", "sweetness": "50%", "toppings": [], "qty": 1, "note": ""
        }
        self.add(item)
...
class FancyLogin(tk.Tk):
    """หน้าล็อกอินเปิดแรก — หลัง Sign In จะเปิด App() และปิดหน้าต่างนี้"""
    def __init__(self):
        super().__init__()
    ...
    def _make_register(self):
        ...
        self.reg_name  = self._field(self.card, "NAME")
    ...
    def sign_in(self):
        ...
        if row:
            ...
            # เปิดแอปหลัก แล้วปิดหน้าล็อกอินนี้
            self.destroy()
            app = App()
            app.login_ok(row)
            app.mainloop()
    ...
    def register(self):
        ...
        if not u or not p1 or not p2:
            messagebox.showerror("Error", "Please fill username/password."); return
        if p1 != p2:
            messagebox.showerror("Error", "Passwords do not match."); return
        if not valid_username(u):
            messagebox.showerror("Error","Invalid username"); return
        if not valid_password(p1):
            messagebox.showerror("Error","Password must be at least 6 chars"); return
        if not valid_email(em):
            messagebox.showerror("Error","Invalid email"); return
        if not valid_phone(ph):
            messagebox.showerror("Error","Invalid phone"); return
        ok, msg = self.db.create_user(u, p1, phone=ph, email=em, name=nm)
        if ok:
