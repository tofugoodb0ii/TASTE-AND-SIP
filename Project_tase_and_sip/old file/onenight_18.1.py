...
# แก้ให้เป็นไฟล์ภายในโปรเจ็กต์ วางรูป qrcode.jpg ไว้ที่ assets/images/qrcode.jpg
...
class DB:
    ...
    def report_gross_margin(self, start, end):
        # คงไว้เพื่อความเข้ากันได้ แต่ไม่เรียกใช้จาก UI แล้ว
        return self.conn.execute("""
            WITH sold AS (
...
class ShopView(ttk.Frame):
    def __init__(self, master, app: App):
        ...
        # ====== แบ่ง 2 คอลัมน์: QR (ซ้าย) | Slip Upload (ขวา) ======
    ...
    def _build_tabs(self):
        ...
        for cat in self.app.db.categories():
            ...
            for i,p in enumerate(self.app.db.products_by_cat(cat['id'])):
                ...
                # Simple product card (ProductCard not defined)
                ...
                if img:
                    lbl_img = ttk.Label(card, image=img)
...
class AdminView(ttk.Frame):
    ...
    def on_show(self):
        ...
        # ไม่ต้องเรียก inventory / banner อีก
