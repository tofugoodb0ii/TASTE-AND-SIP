...
IMG_QR_PATH      = os.path.join(IMG_DIR, "qrcode.jpg")  # วาง qrcode.jpg ไว้ใน assets/images/
...
class DB:
    ...
    def upsert_product(self, pid, name, category_id, base_price, image, is_active):
        ...
        if pid:
            cur.execute("""UPDATE products
                           ...
                           WHERE id=?""",
                        (name, category_id, base_price, image, is_active, pid))
        else:
            cur.execute("""INSERT INTO products(name,category_id,base_price,image,is_active)
                           VALUES(?,?,?,?,?)""",
                        (name, category_id, base_price, image, is_active))
        self.conn.commit()
    ...
    def create_order(self, user_id, cart_items, promo_code, payment_method="SLIP", payment_ref=""):
        ...
        cur.execute("""INSERT INTO orders(user_id,order_datetime,channel,pickup_time,subtotal,discount,total,payment_method,status,vat)
                       VALUES(?,?,?,?,?,?,?,?,?,?)""",
                    (user_id, now_ts(), "", "", subtotal, discount, total, payment_method, "PENDING_PAYMENT", vat))
...
class AuthApp(ctk.CTk):
    ...
    def __init__(self, db_path: str = DB_FILE, left_bg_path: Optional[str] = None,
        ...
        # ทำให้ฟอร์ม "ลอย" บนพื้นหลัง (ไม่มีกล่องการ์ด)
        self.card = ctk.CTkFrame(self.right, fg_color="transparent", corner_radius=0,
                                 border_color=None, border_width=0, width=660, height=560)
...
class CTkMain(ctk.CTk):
    ...
    def go(self, key: str, replace: bool=False):
        ...
        # --- RBAC guard ---
        if key == "admin":
            if not self.user or (self.user.get("role") or "") != "admin":
    ...
    def login_ok(self, user_row):
        self.user = user_row
        # avatar + username (มี default ถ้าไม่พบไฟล์)
        avatar_path = user_row.get("avatar") if user_row.get("avatar") and os.path.exists(user_row.get("avatar")) else None
        img = ctk_img(avatar_path, (32,32))
...
class ProfilePage(ctk.CTkFrame):
    """โปรไฟล์แก้ไขข้อมูล: 2 คอลัมน์, อัปโหลดรูปซ้าย, label อยู่บนช่อง"""
    def __init__(self, app: CTkMain):
        super().__init__(app.body, fg_color=PALETTE["antique"]); self.app=app
        ...
        right.grid(row=0, column=1, sticky="ew", padx=12, pady=12)
        right.grid_columnconfigure(0, weight=1)

        def field(row, label, show=""):
            ctk.CTkLabel(right, text=label, text_color=PALETTE["bistre"]).grid(row=row, column=0, sticky="w", pady=(6,2))
            e = ctk.CTkEntry(right, show=show); e.grid(row=row+1, column=0, sticky="ew")
            return e

        self.e_name  = field(0, "Username")
        self.e_pass  = field(2, "Password (leave blank to keep)", show="•")
        self.e_phone = field(4, "Phone Number")
        self.e_birth = field(6, "Date of Birth (YYYY-MM-DD)")

        ctk.CTkButton(right, text="Save Change", corner_radius=18,
                      fg_color=PALETTE["white"], text_color=PALETTE["bistre"],
                      command=self.save).grid(row=8, column=0, sticky="e", pady=12)
...
class CategoryPage(ctk.CTkFrame):
    """Grid การ์ดสินค้า + Search + Back/Home/Cart"""
    def __init__(self, app: CTkMain):
        super().__init__(app.body, fg_color=PALETTE["antique"]); self.app=app
    ...
    def render(self):
        ...
        for p in products:
            ...
            ctk.CTkLabel(card, text=p["name"], font=ctk.CTkFont(size=14, weight="bold"),
                         text_color=PALETTE["bistre"], wraplength=280).pack()
...
class CartPage(ctk.CTkFrame):
    ...
    def __init__(self, app: CTkMain):
        ...
        self.e_code.bind("<KeyRelease>", lambda e: self._update_totals())
    ...
    def render_list(self):
        ...
        for it in self.app.cart:
            ...
            ctk.CTkLabel(row, text=it["name"], width=260, anchor="w",
                         text_color=PALETTE["bistre"], wraplength=240).pack(side="left", padx=8, pady=8)
...
class AdminOrders(ctk.CTkFrame):
    def __init__(self, master, db: DB):
        ...
        self.cb = ctk.CTkComboBox(ctr, values=["PAID","PREPARING","READY","COMPLETED","CANCELLED","PENDING_PAYMENT"]); self.cb.pack(side="left", padx=6)
    ...
    def show_detail(self, oid):
        ...
        self.cb.set(o['status'] or "PENDING_PAYMENT")
    ...
    def set_status(self):
        if not self.selected_oid: return
        st = (self.cb.get() or "PENDING_PAYMENT").upper()
