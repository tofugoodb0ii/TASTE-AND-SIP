...
# - ปรับ Forgot Password ให้เป็น 2 ขั้นตอน (Verify -> ตั้งรหัสใหม่) และไม่ล้นกรอบ
...
class OrdersView(ttk.Frame):
    def __init__(self, master, app: App):
        ...
        ttk.Button(fr,text="Download Receipt",command=self.open).pack(side="right",padx=4)  # เปลี่ยนปุ่มเป็น Download
    ...
    def open(self):
        ...
        path=create_receipt_pdf(oid,self.app.db,self.app.user)  # สร้างทุกครั้ง เพื่อให้เป็นเทมเพลตใหม่เสมอ
        try:
            if sys.platform.startswith("win"): os.startfile(path)
            elif sys.platform=="darwin": os.system(f"open '{path}'")
            else: os.system(f"xdg-open '{path}'")
