...
# - Fancy Login (left image + logo) -> Full App (Shop, Cart, Options Size/Spiciness/Meat/Toppings, Promo, QR Pay, PDF Receipt)
...
class DB:
    ...
    def _seed(self):
        ...
        # ===== options (Size, Spiciness, Meat) =====
        size_json  = json.dumps({"values":["S","M","L"],"price_multipliers":{"S":1.0,"M":1.2,"L":1.5}})
        spice_json = json.dumps({"values":["ไม่เผ็ด","เผ็ดน้อย","เผ็ดกลาง","เผ็ดมาก","เผ็ดสุด"]})
        meat_json  = json.dumps({"values":["หมู","ไก่","เนื้อ","กุ้ง","เต้าหู้"]})
        ...
        for p in c.execute("SELECT id FROM products"):
            ...
            # Sweetness -> Spiciness (อัปเดตถ้ามีของเดิม)
            if c.execute("SELECT 1 FROM product_options WHERE product_id=? AND option_name='Sweetness'",(p['id'],)).fetchone():
                c.execute("""UPDATE product_options
                             SET option_name='Spiciness', option_values_json=?
                             WHERE product_id=? AND option_name='Sweetness'""", (spice_json, p['id']))
            elif not c.execute("SELECT 1 FROM product_options WHERE product_id=? AND option_name='Spiciness'",(p['id'],)).fetchone():
                c.execute("INSERT INTO product_options(product_id,option_name,option_values_json) VALUES(?,?,?)",(p['id'],"Spiciness",spice_json))
            # Meat
            if not c.execute("SELECT 1 FROM product_options WHERE product_id=? AND option_name='Meat'",(p['id'],)).fetchone():
                c.execute("INSERT INTO product_options(product_id,option_name,option_values_json) VALUES(?,?,?)",(p['id'],"Meat",meat_json))
    ...
    def create_order(self, user_id, channel, pickup_time, cart_items, promo_code, payment_method="QR"):
        ...
        # items
        for it in cart_items:
            opts = {"Size": it.get("size"), "Spiciness": it.get("spiciness") or it.get("sweetness"), "Meat": it.get("meat")}
...
def create_receipt_pdf(order_id, db: DB, user_row):
    ...
    for it in items:
        ...
        sp = opts.get('Spiciness') or opts.get('Sweetness','-')  # รองรับออเดอร์เก่า
        me = opts.get('Meat','-')
        canv.drawString(x+8*mm, y, f"Size: {opts.get('Size','-')}, Spiciness: {sp}, Meat: {me}"); y -= 5*mm
        for tp in tps:
...
class ProductDialog(tk.Toplevel):
    def __init__(self, master, db: DB, prod_row, toppings, on_add):
        ...
        self.geometry("420x600"); self.resizable(False, False)
        opts = db.product_options(prod_row['id'])
        self.size_cfg  = opts.get('Size', {"values":["S","M","L"],"price_multipliers":{"S":1.0,"M":1.2,"L":1.5}})
        self.spice_cfg = opts.get('Spiciness', {"values":["ไม่เผ็ด","เผ็ดน้อย","เผ็ดกลาง","เผ็ดมาก","เผ็ดสุด"]})
        self.meat_cfg  = opts.get('Meat', {"values":["หมู","ไก่","เนื้อ","กุ้ง","เต้าหู้"]})
        ...
        ttk.Label(frm, text="Spiciness").pack(anchor="w")
        self.var_spice = tk.StringVar(value=self.spice_cfg["values"][0])
        ttk.Combobox(frm, values=self.spice_cfg["values"], textvariable=self.var_spice, state="readonly").pack(fill="x", pady=4)

        ttk.Label(frm, text="Meat").pack(anchor="w")
        self.var_meat = tk.StringVar(value=self.meat_cfg["values"][0])
        ttk.Combobox(frm, values=self.meat_cfg["values"], textvariable=self.var_meat, state="readonly").pack(fill="x", pady=4)
    ...
    def _add(self):
        ...
        item = {
            ...
            "spiciness": self.var_spice.get(),
            "meat": self.var_meat.get(),
...
class ShopView(ttk.Frame):
    ...
    def refresh(self):
        ...
        for it in self.app.cart:
            ...
            tops=sum(t['price']*t.get('qty',1) for t in it.get('toppings',[]))
            line=(base+tops)*it['qty']
            self.tv.insert("", "end", values=(f"{it['name']} [{it.get('size','-')}/{it.get('spiciness','-')}/{it.get('meat','-')}]", it['qty'], f"{line:.2f}"))
