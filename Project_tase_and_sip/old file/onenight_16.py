...
# ===== Glue: ใช้ AuthApp ล็อกอิน แล้วเปิด App (Tkinter) ต่อ =====
def _start_main_app(user_row):
    main_app = App()
    # รีเฟรช user จาก DB หลักให้แน่ใจว่า schema/table อื่นถูก seed แล้ว
    # (กรณีล็อกอินมาจาก DB ที่ยังมีแค่ตาราง users)
...
if __name__ == "__main__":
    ensure_dirs()  # สร้างโฟลเดอร์ assets/reports ต่างๆ
    def _on_login_success(row):
        # ปิดหน้าล็อกอินก่อน แล้วค่อยเปิดแอปหลัก
        auth.destroy()
        _start_main_app(row)
