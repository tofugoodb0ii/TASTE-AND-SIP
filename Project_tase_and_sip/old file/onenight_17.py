# -*- coding: utf-8 -*-
# TASTE AND SIP (Single file, AuthApp -> App glue)
# - Fancy Login (customtkinter, left image + logo) -> Full App (Shop, Cart, Options, Promo, QR Pay, PDF Receipt)
# - Admin: Products, Inventory (BOM & low stock), Promotions, Reports
...
IMG_QR_PATH         = os.path.join(IMG_DIR, r"C:\Users\thatt\Downloads\qrcode.jpg")       # <- วางรูปคิวอาร์เอง
...
class App(tk.Tk):
    def __init__(self, on_logout_to_auth: Optional[Callable[[], None]] = None):
        ...
        self.user = None
        self.cart = []
        self.on_logout_to_auth = on_logout_to_auth  # <— callback กลับไป AuthApp
        self._styles()
        self._layout()
    ...
    def logout(self):
        # เคลียร์สถานะ แล้วปิดหน้าต่างหลัก จากนั้นกลับไป AuthApp (รูปแรก)
        self.user=None; self.cart.clear()
        try:
            self.destroy()
        finally:
            if self.on_logout_to_auth:
                self.on_logout_to_auth()
...
# ===== Glue: สลับไปมาระหว่าง AuthApp และ App =====
def _start_main_app(user_row):
    # เปิดหน้าหลัก และกำหนดให้ Logout กลับไป AuthApp
    def launch_auth():
        _launch_auth()
