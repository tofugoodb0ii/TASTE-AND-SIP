...
class DB:
    ...
    def upsert_promotion(self, pid, code, ptype, value, min_spend, start_at, end_at, applies_to_product_id, is_active):
        ...
        if pid:
            ...
            # แก้บั๊กจำนวนคอลัมน์/placeholder ให้ตรงกัน
    ...
    # ==== เพิ่มใหม่: สรุปยอดขายรายเดือน/รายปี ====
    def report_sales_monthly(self, start, end):
        """สรุปต่อเดือน: period, revenue, orders, aov (นับเฉพาะออเดอร์ที่ไม่ยกเลิก)"""
        return self.conn.execute("""
    ...
    def report_sales_yearly(self, start, end):
        """สรุปต่อปี: period, revenue, orders, aov (นับเฉพาะออเดอร์ที่ไม่ยกเลิก)"""
        return self.conn.execute("""
...
class ShopView(ttk.Frame):
    def __init__(self, master, app: App):
        ...
        qr_wrap = ttk.LabelFrame(pay, text="Scan to Pay (TASTE AND SIP)")
...
class AdminView(ttk.Frame):
    ...
    def on_show(self):
        ...
        self.reload_products(); self._admin_orders() ; self.reload_promos()
    ...
    def _admin_orders(self):
        ...
        for k,w,anc in [("menu",220,"w"),("qty",60,"e"),("unit",90,"e"),("total",100,"e")]:
    ...
    def _reports(self):
        ...
        # === ใหม่: รายเดือน/รายปี ===
        ttk.Button(fr,text="By Month",command=self.run_monthly).grid(row=0,column=8,padx=(18,6))
        ttk.Button(fr,text="By Year",command=self.run_yearly).grid(row=0,column=9,padx=6)
    ...
    # === ใหม่: ปุ่มเรียกสรุปยอดรายเดือน/รายปี ===
    def run_monthly(self):
        s=self.var_start.get().strip(); e=self.var_end.get().strip()
        data=self.app.db.report_sales_monthly(s,e)
    ...
    def run_yearly(self):
        s=self.var_start.get().strip(); e=self.var_end.get().strip()
        data=self.app.db.report_sales_yearly(s,e)
        rows=[(r['period'], f"{(r['revenue'] or 0):.2f}", int(r['orders'] or 0), f"{(r['aov'] or 0):.2f}") for r in data]
...
def _launch_auth():
    # ensure DB + admin seed ก่อนแสดงหน้าล็อกอิน
    _ = DB()  # เรียกใช้เพื่อให้ _schema/_seed ทำงาน
