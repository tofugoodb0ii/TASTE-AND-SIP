# -*- coding: utf-8 -*-
# TASTE AND SIP (Customer-first single file)
# - Fancy Login (customtkinter) -> Main App (Shop, Cart dialog, Checkout/Payment)
# - Admin (Products, Promotions, basic Reports) — hidden unless role="admin"
# - No inventory / no gross margin
# - Orders include VAT 7% and optional slip image
# - Receipt PDF has VAT line
#
# Folders auto-created: assets/images (qr.png), assets/avatars, assets/images/products, reports
...
# ---------- (เปลี่ยน path รูปฝั่ง login ถ้าต้องการ) ----------
LEFT_BG_PATH = r"C:\Users\thatt\OneDrive\Desktop\python\Project_tase_and_sip\image\A vibrant landscape of a traditional restaurant with tables filled with a wide variety of national foods and colorful drinks. The scene is festive and welcoming, with beautifully arranged dishes from different cult.jpg"  # BG ซ้าย
LOGO_PATH    = r"C:\Users\thatt\OneDrive\Desktop\python\Project_tase_and_sip\image\lologogo.png-removebg-preview.png" # Logo sample
...
class DB:
    ...
    def _schema(self):
        ...
        # simple options table left here but not used in UI
        c.execute("""
        ...
        CREATE TABLE IF NOT EXISTS orders(
            ...
            promo_code TEXT, slip_path TEXT
        )""")
    ...
    def find_promo(self, code):
        ...
        try:
            ...
            if not (st <= dt.now() <= ed): return None
        except: pass
        return row

    # --- orders ---
    def create_order(self, user_id:int, cart_items:list, promo_code:str, slip_path:str, payment_method="SLIP"):
        # compute totals
        subtotal = sum(float(it['base_price']) * int(it['qty']) for it in cart_items)
        discount = 0.0
        promo = self.find_promo((promo_code or "").strip().upper())
        if promo:
            ...
            if ptype in ("PERCENT_BILL","FLAT_BILL") and subtotal >= float(promo['min_spend'] or 0):
                ...
                target=sum(float(it['base_price'])*int(it['qty']) for it in cart_items if it['product_id']==pid)
        ...
        cur.execute("""INSERT INTO orders(user_id,order_datetime,subtotal,discount,vat,total,payment_method,status,promo_code,slip_path)
                       VALUES(?,?,?,?,?,?,?,?,?,?)""",
                    (user_id, now_ts(), subtotal, discount, vat, total, payment_method, "PAID", (promo_code or "").upper(), slip_path or ""))
        order_id = cur.lastrowid
        for it in cart_items:
            cur.execute("""INSERT INTO order_items(order_id,product_id,qty,unit_price) VALUES(?,?,?,?)""",
                        (order_id, it['product_id'], int(it['qty']), float(it['base_price'])))
...
def create_receipt_pdf(order_id, db: DB, user_row):
    ...
    items = db.conn.execute("""SELECT oi.*, p.name AS product_name
    ...
    canv.setFont("Helvetica", 10)
    for it in items:
        canv.drawString(x, y, f"- {it['product_name']} x{it['qty']} @ {money(it['unit_price'])}")
        y -= 5*mm

    y -= 4*mm
    canv.setFont("Helvetica-Bold", 11)
    canv.drawString(x, y, f"Subtotal: {money(order['subtotal'])}"); y -= 5*mm
    canv.drawString(x, y, f"Discount: {money(order['discount'])}"); y -= 5*mm
    canv.drawString(x, y, f"VAT 7%: {money(order['vat'])}"); y -= 5*mm
    canv.drawString(x, y, f"Total: {money(order['total'])}"); y -= 8*mm
    canv.setFont("Helvetica", 10)
    canv.drawString(x, y, f"Payment: {order['payment_method']}   Status: {order['status']}"); y -= 6*mm
    canv.drawString(x, y, f"Promo: {order['promo_code'] or '-'}")
...
# ======================= MAIN APP =======================
class App(tk.Tk):
    def __init__(self, boot_from_auth=False):
        super().__init__()
        ...
        self._styles()
        self._layout()
        self._boot_from_auth = boot_from_auth  # ถ้า logout แล้วอยากกลับไปหน้า Auth
    ...
    def _layout(self):
        ...
        ttk.Button(top, text="Checkout", command=self._quick_checkout).pack(side="left", padx=4)

        # Admin button (ซ่อนค่า default)
        self.btn_admin = ttk.Button(top, text="Admin", command=lambda:self.show("Admin"))
        # ไม่ pack ตอนเริ่ม — จะ pack เมื่อ login_admin
    ...
    def logout(self):
        ...
        if self.btn_admin.winfo_ismapped(): self.btn_admin.pack_forget()
        # กลับไปหน้า Login (รวมถึงกรณีเปิดจาก Auth)
        self.show("Login")
        if self._boot_from_auth:
            # ปิด Main app แล้วเรียกหน้า Auth ใหม่
            self.after(100, self._relaunch_auth)

    def _relaunch_auth(self):
        try: self.destroy()
        finally:
            launch_auth_then_app()

    # --- Cart / Checkout shortcuts ---
    def open_cart(self):
        dlg = getattr(self, "_cart_dlg", None)
    ...
    def _quick_checkout(self):
        if not self.cart:
            messagebox.showinfo("Checkout","Cart is empty"); return
        self.open_cart()
        if hasattr(self, "_cart_dlg") and self._cart_dlg.winfo_exists():
            self._cart_dlg.open_payment()
...
class ShopView(ttk.Frame):
    def __init__(self, master, app: App):
        ...
        ttk.Label(self, text=" ", width=2).pack(side="left")  # spacer
...
class OrdersView(ttk.Frame):
    def __init__(self, master, app: App):
        ...
        for k,w in [("id",80),("date",200),("total",90)]:
...
class AdminView(ttk.Frame):
    ...
    def _reports(self):
        ...
        self.tvr=ttk.Treeview(self.tab_reports,columns=("date","total"),show="headings")
        self.tvr.heading("date", text="DATE"); self.tvr.heading("total", text="TOTAL")
        self.tvr.column("date",width=140); self.tvr.column("total",width=140)
    ...
    def run_daily(self):
        ...
        """,(s+" 00:00:00", e+" 23:59:59")).fetchall()
        for i in self.tvr.get_children(): self.tvr.delete(i)
        for r in rows: self.tvr.insert("", "end", values=(r['d'], f"{(r['total'] or 0):.2f}"))
...
class CartDialog(tk.Toplevel):
    def __init__(self, app: App, db: DB):
        ...
        promo=ttk.Frame(self); promo.pack(fill="x", padx=12, pady=(8,0))
        ttk.Label(promo,text="Promo Code").pack(side="left")
        self.var_code=tk.StringVar(); ttk.Entry(promo,textvariable=self.var_code,width=16).pack(side="left", padx=6)
        ttk.Button(promo,text="Apply",command=self.refresh_totals).pack(side="left")

        summary=ttk.Frame(self); summary.pack(fill="x", padx=12, pady=(10,0))
        self.lbl_sub=ttk.Label(summary,text="Subtotal: 0.00"); self.lbl_sub.pack(anchor="e")
        self.lbl_dis=ttk.Label(summary,text="Discount: 0.00"); self.lbl_dis.pack(anchor="e")
        self.lbl_vat=ttk.Label(summary,text="VAT 7%: 0.00"); self.lbl_vat.pack(anchor="e")
        self.lbl_tot=ttk.Label(summary,text="Total: 0.00",font=("Segoe UI",11,"bold")); self.lbl_tot.pack(anchor="e")

        bot=ttk.Frame(self); bot.pack(fill="x", padx=12, pady=(12,14))
        ...
        self.bind("<Control-p>", lambda e: self.open_payment())
    ...
    def _sel_index(self):
        sel=self.tv.selection()
        return None if not sel else self.tv.index(sel[0])
    ...
    def save_pdf(self):
        ...
        # สร้าง order แบบ PENDING (ไม่มีสลิป) เพื่อให้มีไฟล์บิล
    ...
    def open_payment(self):
        if not self._cart:
            messagebox.showinfo("Cart","Cart empty")
            return

        def _place_order(slip_path:str):
            oid, s,d,v,t = self.db.create_order(
                self.app.user['id'],
                self._cart,
                self.var_code.get().strip().upper(),
                slip_path=slip_path,
                payment_method="SLIP"
            )
            path = create_receipt_pdf(oid, self.db, self.app.user)
        ...
        def _after_done():
            # เคลียร์ตะกร้าแล้วรีเฟรช
            self._cart.clear()
            self.refresh(self._cart)

        PaymentDialog(self, on_pay=_place_order, on_done=_after_done)

# Payment dialog
class PaymentDialog(tk.Toplevel):
    def __init__(self, master, on_pay:Callable[[str],tuple], on_done:Callable[[],None]):
        super().__init__(master)
        self.title("Checkout & Payment")
        self.geometry("520x600"); self.resizable(False, False)
        self.on_pay = on_pay
        ...
        # QR
        ttk.Label(body, text="Payment: QR", font=("Segoe UI", 11, "bold")).pack(anchor="w")
        self.qr = tk.Canvas(body, width=300, height=300, bg="#fff", highlightthickness=1, highlightbackground="#ddd")
        self.qr.pack(pady=(4, 10))
        self._draw_qr()

        # Slip
        ttk.Label(body, text="Customer Slip", font=("Segoe UI", 11, "bold")).pack(anchor="w", pady=(4,0))
        self.slip = tk.Canvas(body, width=300, height=240, bg="#fff", highlightthickness=1, highlightbackground="#ddd")
        self.slip.pack(pady=(4, 8))
        self._slip_img = None

        # Buttons
        btns = ttk.Frame(body); btns.pack(fill="x")
        ttk.Button(btns, text="Upload Slip", command=self._upload_slip).pack(side="left")
        self.btn_finish = ttk.Button(btns, text="Finish Payment", command=self._finish, state="disabled")
        self.btn_finish.pack(side="right")

        # After paid
        bottom = ttk.Frame(body); bottom.pack(fill="x", pady=(12,0))
        self.btn_download = ttk.Button(bottom, text="Download Receipt (PDF)", command=self._open_receipt, state="disabled")
        self.btn_download.pack(side="left")
        ttk.Button(bottom, text="Done", command=self._done).pack(side="right")

    def _draw_qr(self):
        self.qr.delete("all")
        if os.path.exists(IMG_QR_PATH):
            try:
                img=Image.open(IMG_QR_PATH).convert("RGBA").resize((300,300),Image.LANCZOS)
                self._qrimg=ImageTk.PhotoImage(img); self.qr.create_image(150,150,image=self._qrimg); return
            except: pass
        self.qr.create_text(150,150,text="QR Placeholder\nassets/images/qr.png",justify="center")

    def _upload_slip(self):
        f=filedialog.askopenfilename(filetypes=[("Images","*.png;*.jpg;*.jpeg")])
        if not f: return
        # copy slip into assets to keep
        dest=os.path.join(IMG_DIR, f"slip_{int(dt.now().timestamp())}_{os.path.basename(f)}")
        try:
            shutil.copy2(f, dest)
            self._slip_path = dest
            img = Image.open(dest).convert("RGB")
            img.thumbnail((300, 240), Image.LANCZOS)
            self._slip_img = ImageTk.PhotoImage(img)
            self.slip.delete("all")
            self.slip.create_image(150,120,image=self._slip_img)
            self.btn_finish['state'] = "normal"
        except Exception as e:
            messagebox.showerror("Slip", f"Cannot load slip: {e}")

    def _finish(self):
        try:
            oid, path = self.on_pay(self._slip_path)
            self._receipt_path = path
            self.btn_download['state']="normal"
            messagebox.showinfo("Paid", f"Payment finished. Order #{oid}\nReceipt saved:\n{path}")
        except Exception as e:
            messagebox.showerror("Payment", f"Failed: {e}")

    def _open_receipt(self):
        if not self._receipt_path: return
        try:
            if sys.platform.startswith("win"): os.startfile(self._receipt_path)
            elif sys.platform=="darwin": os.system(f"open '{self._receipt_path}'")
            else: os.system(f"xdg-open '{self._receipt_path}'")
        except:
            messagebox.showinfo("Receipt", f"Saved at: {self._receipt_path}")
...
def validate_username(v: str) -> Optional[str]:
    if not USERNAME_RE.match(v or ""): return "USERNAME MUST BE ≥6 CHARACTERS AND A–Z0–9."
    return None
def validate_phone(v: str) -> Optional[str]:
    if not PHONE_RE.match(v or ""): return "PHONE MUST BE 10 DIGITS."
    return None
def validate_email(v: str) -> Optional[str]:
    if not EMAIL_RE.match(v or ""): return "INVALID EMAIL."
    return None
def validate_password(v: str) -> Optional[str]:
    if not PWD_RE.match(v or ""): return "PASSWORD ≥8, include upper/lower/digit."
    return None
...
class AuthApp(ctk.CTk):
    ...
    def _signin(self):
        ...
        if row:
            # เปิด Main App และปิด Auth
            self.destroy()
            app = App(boot_from_auth=True)
            # sync user กับ DB หลัก
            refreshed = app.db.conn.execute("SELECT * FROM users WHERE id=?", (row['id'],)).fetchone()
            app.login_ok(refreshed or row)
            app.mainloop()
        else:
