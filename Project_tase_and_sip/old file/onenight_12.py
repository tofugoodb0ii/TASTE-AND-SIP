# -*- coding: utf-8 -*-
# TASTE AND SIP (Single file, CTk Login + Tk main app)
# - Login/Sign up/Forgot Password: customtkinter ดีไซน์ครีมมินิมอล
# - หลังล็อกอิน เปิดแอปหลัก Tk (Shop, Cart, Promotions, QR, PDF Receipt, Admin)
# - DB: SQLite, Images via Pillow, PDF via ReportLab

import os, sys, json, sqlite3, hashlib, shutil, re, math
...
# ---------- Login screen images (แก้ path ให้ตรงเครื่อง) ----------
...
IMG_QR_PATH         = os.path.join(IMG_DIR, "qr.png")
...
# ---------- Helpers: tk.PhotoImage ----------
def _tk_load(path):
    try:
        if os.path.exists(path): return tk.PhotoImage(file=path)
    except Exception:
        pass
    return None
...
def _tk_load_and_cover(path, box_w, box_h): return _tk_cover(_tk_load(path), box_w, box_h)
...
class DB:
    ...
    def _schema(self):
        ...
        CREATE TABLE IF NOT EXISTS promotions(
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            code TEXT UNIQUE,
            type TEXT, value REAL, min_spend REAL DEFAULT 0,
        ...
        c.execute("""CREATE TABLE IF NOT EXISTS order_item_toppings(
    ...
    # --- user/auth/profile ---
    def create_user(self, username, password, phone=None, email=None):
        try:
            self.conn.execute(
    ...
    def change_password(self, uid_or_username, newpass, by_username=False):
        if by_username:
            self.conn.execute("UPDATE users SET password_hash=? WHERE username=?", (sha256(newpass), uid_or_username))
        else:
            self.conn.execute("UPDATE users SET password_hash=? WHERE id=?", (sha256(newpass), uid_or_username))
        self.conn.commit()
    ...
    def available_stock(self, product_id: int, size: str | None) -> int:
        """คำนวณจำนวนเสิร์ฟที่ทำได้จาก BOM + คูณ multiplier ของ Size"""
        opts = self.product_options(product_id) or {}
        mult = 1.0
        if size and "Size" in opts:
            mult = float((opts["Size"].get("price_multipliers") or {}).get(size, 1.0))
        rows = self.conn.execute("""
            SELECT bl.qty_per_unit, ii.qty_on_hand
            FROM bom_links bl
        ...
        cap = math.inf
        for r in rows:
            need = float(r["qty_per_unit"]) * mult
            if need <= 0: continue
            possible = float(r["qty_on_hand"]) / need
            cap = min(cap, math.floor(possible))
        if cap is math.inf: return 0
        return max(0, int(cap))

    def deduct_for_order(self, order_id):
        rows = self.conn.execute("""
            SELECT oi.id AS oi_id, oi.qty, oi.options_json, bl.inventory_item_id, bl.qty_per_unit
            FROM order_items oi
            JOIN bom_links bl ON bl.product_id = oi.product_id
            WHERE oi.order_id=?
        """,(order_id,)).fetchall()
        size_mult_map = {"S":1.0,"M":1.2,"L":1.5}
        cur = self.conn.cursor()
        for r in rows:
            size = None
            try:
                size = (json.loads(r["options_json"] or "{}") or {}).get("Size")
            except Exception:
                size = None
            mult = size_mult_map.get(size, 1.0)
            dec = float(r['qty']) * float(r['qty_per_unit']) * mult
    ...
    def list_promotions(self): return self.conn.execute("SELECT * FROM promotions ORDER BY id DESC").fetchall()
...
class ProductDialog(tk.Toplevel):
    def __init__(self, master, db: DB, prod_row, toppings, on_add=None):
        super().__init__(master); self.title(f"Customize - {prod_row['name']}")
        self.db=db; self.prod=prod_row; self.toppings=toppings
        ...
        self.var_size = tk.StringVar(value=self.size_cfg["values"][0] if self.size_cfg.get("values") else "")
        ttk.Combobox(frm, values=self.size_cfg.get("values", []), textvariable=self.var_size, state="readonly").pack(fill="x", pady=4)

        ttk.Label(frm, text="Sweetness").pack(anchor="w")
        default_sweet = self.sweet_cfg.get("values", [""])[2] if len(self.sweet_cfg.get("values", []))>2 else (self.sweet_cfg.get("values", [""])[0] if self.sweet_cfg.get("values") else "")
        self.var_sweet = tk.StringVar(value=default_sweet)
        ttk.Combobox(frm, values=self.sweet_cfg.get("values", []), textvariable=self.var_sweet, state="readonly").pack(fill="x", pady=4)
        ...
        # ให้ dialog เข้าถึง app ผ่าน master
        self.app = master.app
...
class App(tk.Tk):
    def __init__(self):
        ...
        except Exception: self.geometry("1280x800")
        ensure_dirs()
        self.db = DB()
        self.user = None
        self.cart = []
    ...
    def logout(self):
        ...
        if hasattr(self, "_back_to_ctk_login") and callable(self._back_to_ctk_login):
            self._back_to_ctk_login()
        else:
            try: self.destroy()
            finally: launch_ctk_login(existing_db=getattr(self, "db", None))

    # ---------- เพิ่มสินค้าลงตะกร้า ----------
    def cart_add(self, product_id: int, name: str, base_price: float,
                 size: str | None, sweetness: str | None,
                 toppings: list, qty: int = 1, note: str = "",
                 image: str = "", go_cart: bool = False, require_size: bool = True) -> bool:
        if not self.user:
            messagebox.showerror("Add to Cart", "กรุณาล็อกอินก่อนเพิ่มสินค้าลงตะกร้า")
            return False
        if require_size and (size is None or str(size).strip() == ""):
            messagebox.showerror("Add to Cart", "โปรดเลือกไซส์ก่อนเพิ่มสินค้าลงตะกร้า")
            return False
        opts = self.db.product_options(int(product_id))
        valid_sizes = (opts.get("Size", {}) or {}).get("values", [])
        if require_size and valid_sizes and size not in valid_sizes:
            messagebox.showerror("Add to Cart", f"ขนาด {size} ไม่มีในฐานข้อมูล")
            return False

        available = self.db.available_stock(int(product_id), size)
        in_cart = 0
        for it in self.cart:
            if it["product_id"] == int(product_id) and (it.get("size") == size):
                in_cart += int(it.get("qty", 0))
        if in_cart + qty > available:
            remain = max(0, available - in_cart)
            msg = f"สินค้าหมดสำหรับไซส์ {size}" if remain <= 0 else f"สต็อกไม่เพียงพอ (เหลือ {remain} ชุดสำหรับไซส์ {size})"
            messagebox.showerror("Add to Cart", msg)
            return False

        merged = False
        for it in self.cart:
            if it["product_id"] == int(product_id) and it.get("size") == size and it.get("sweetness") == sweetness and it.get("note","") == (note or "") and self._same_toppings(it.get("toppings",[]), toppings):
                it["qty"] += int(qty); merged = True; break
        if not merged:
            self.cart.append({
        ...
        if "ShopView" in self.frames:
            try: self.frames["ShopView"].refresh()
            except Exception: pass

        messagebox.showinfo("Add to Cart", "เพิ่มลงตะกร้าเรียบร้อย")
        if go_cart: self.show("Shop")
        return True

    @staticmethod
    def _same_toppings(t1, t2):
        def norm(xs):
            return sorted([(int(x["id"]), int(x.get("qty",1)), float(x.get("price",0))) for x in xs], key=lambda z:(z[0],z[1],z[2]))
        return norm(t1 or []) == norm(t2 or [])

# ---------- Views (fallback/simple inside main app) ----------
    ...
    def _build_tabs(self):
        ...
        for cat in self.app.db.categories():
            ...
            for i,p in enumerate(self.app.db.products_by_cat(cat['id'])):
                ...
                ProductCard(rowf, dict(p), self.customize).grid(row=0,column=col,padx=6); col+=1

    def customize(self, prod_row):
        ProductDialog(self, self.app.db, prod_row, self.app.db.toppings())
...
def _val_username(v): return None if USERNAME_RE.match(v or "") else "USERNAME MUST BE AT LEAST 6 CHARACTERS AND CONTAIN ONLY A–Z AND 0–9."
def _val_phone(v):    return None if PHONE_RE.match(v or "")    else "PHONE MUST BE 10 DIGITS."
def _val_email(v):    return None if EMAIL_RE.match(v or "")    else "INVALID EMAIL FORMAT."
def _val_pwd(v):      return None if PWD_RE.match(v or "")      else "PASSWORD MUST BE ≥ 8 CHARS, INCLUDE UPPER, LOWER AND A DIGIT."
...
# ---------- Helper: เปิดหน้า CTk Login ใหม่ ----------
def launch_ctk_login(existing_db=None):
    ensure_dirs()
    login = CTkLogin(db_path=DB_FILE, left_bg_path=LEFT_BG_PATH, logo_path=LOGO_PATH)
    if existing_db is not None: login.db = existing_db
    login.mainloop()

class CTkLogin(ctk.CTk):
    def __init__(self, db_path: str = DB_FILE, left_bg_path: str = LEFT_BG_PATH, logo_path: str = LOGO_PATH):
        super().__init__()
    ...
    def show_forgot(self):
        ...
        self.fp_step2 = ctk.CTkFrame(self.card, fg_color="transparent"); self.fp_step2.pack_forget()
        self.fp_pwd1 = LabeledEntry(self.fp_step2, "NEW PASSWORD", show="•")
        self.fp_pwd2 = LabeledEntry(self.fp_step2, "CONFIRM NEW PASSWORD", show="•")
        SubmitBtn(self.fp_step2, "CHANGE PASSWORD", command=self._forgot_change)
        LinkBtn(self.card, "BACK TO LOGIN", command=self.show_signin).pack(pady=(0, 18))
    ...
    def _signin(self):
        ...
        if row:
            ...
            def _back_to_login():
                ...
                launch_ctk_login(existing_db=self.db)
            app._back_to_ctk_login = _back_to_login
            app.mainloop()
        else:
    ...
    def _signup(self):
        ...
        for fn in (lambda: _val_username(u), lambda: _val_phone(ph), lambda: _val_email(em), lambda: _val_pwd(p1)):
            msg = fn()
            if msg: self.su_err.set(msg); return
        if p1 != p2:
            self.su_err.set("PASSWORDS DO NOT MATCH."); return
        try:
            ok, msg = self.db.create_user(u, p1, ph, em)
            self.su_err.set("ACCOUNT CREATED. PLEASE SIGN IN." if ok else msg)
        except sqlite3.IntegrityError:
    ...
    def _forgot_verify(self):
        ...
        if row:
            ...
            self.fp_step2.pack(fill="x", padx=24, pady=(6, 12))
            self.fp_pwd1.pack(fill="x", padx=12, pady=(6, 8))
            self.fp_pwd2.pack(fill="x", padx=12, pady=(6, 10))
            for w in self.fp_step2.winfo_children():
                if isinstance(w, SubmitBtn): w.pack_forget()
    ...
    def _forgot_change(self):
        ...
        try:
            self.db.change_password(self._verified_username, p1, by_username=True)
