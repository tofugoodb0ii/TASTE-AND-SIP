...
PW_RE   = re.compile(r'^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[A-Za-z0-9]{9,}$')

def valid_username(u: str) -> bool:
    return bool(USER_RE.fullmatch(u or ""))

def valid_password(p: str) -> bool:
    # ≥9 ตัว, มีตัวเล็ก/ใหญ่/ตัวเลข อย่างน้อยอย่างละ 1 และมีได้เฉพาะ A-Za-z0-9
    return bool(PW_RE.fullmatch(p or ""))
...
class DB:
    ...
    # --- user/auth/profile ---
    def create_user(self, username, password, phone="", email="", name=""):
        if not valid_username(username):
            return False, "Invalid username: letters and numbers only."
        if not valid_password(password):
            return False, "Invalid password: ≥9 chars, include A-Z, a-z, and 0-9, no special chars."
        try:
            self.conn.execute(
                "INSERT INTO users(username,password_hash,name,phone,email,role) VALUES(?,?,?,?,?,?)",
                (username, sha256(password), name.strip(), phone.strip(), email.strip(), "customer")
            )
            self.conn.commit()
...
class RegisterView(ttk.Frame):
    def __init__(self, master, app: App):
        ...
        self.ephone=self._row(frm,"Phone",1)
        self.eemail=self._row(frm,"Email",2)
        self.ep1=self._row(frm,"Password",3,show="•")
        self.ep2=self._row(frm,"Confirm",4,show="•")
    ...
    def reg(self):
        ...
        ph=self.ephone.get().strip()
        em=self.eemail.get().strip()
        p1=self.ep1.get().strip(); p2=self.ep2.get().strip()
        ...
        if not valid_username(u):
            messagebox.showerror("Error","Username must contain only A-Z, a-z, 0-9."); return
        if not valid_password(p1):
            messagebox.showerror("Error","Password must be ≥9 chars, include A-Z, a-z, 0-9, and no special chars."); return
        ok,msg=self.app.db.create_user(u,p1,ph,em,"")
...
# ======================= FANCY LOGIN (ย่อขนาด + ขยับขึ้นบน ไม่มีสกรอลล์) =======================
class FancyLogin(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title(APP_TITLE)
        self.state("zoomed")  # คงเดิม
        ...
        # ย่อข้อความฝั่งซ้ายลงเล็กน้อย
        canvas.create_text(32, 28, text="WELCOME TO \nTASTE AND SIP", fill="white",
                           font=("Georgia", 28, "bold"), anchor="nw")
        canvas.create_text(36, 78, text="\n\nFOOD AND DRINK !", fill="white",
                           font=("Georgia", 14), anchor="nw")

        # -------- ขวา: ย่อโลโก้ + การ์ด + ลดช่องไฟ และย้ายขึ้นบน --------
        right = tk.Frame(self, bg="#f8eedb")
        right.pack(side="left", fill="both", expand=True)

        # โลโก้เล็กลง
        self.logo_img = _tk_load_and_fit(LOGO_PATH, 160, 160)  # เดิม 220 -> 160
        logo_wrap = tk.Frame(right, bg="#f8eedb")
        logo_wrap.pack(pady=(8, 2), anchor="n")  # ดันขึ้นบนและลดช่องไฟ
        if self.logo_img:
            tk.Label(logo_wrap, image=self.logo_img, bg="#f8eedb").pack()
        else:
            tk.Label(logo_wrap, text="TASTE AND SIP", bg="#f8eedb",
                     font=("Segoe UI", 18, "bold")).pack()

        # การ์ดสมัคร/ล็อกอิน: เล็กลงและอยู่สูงขึ้น
        self.card = tk.Frame(right, bg="#ffffff")
        self.card.pack(padx=28, pady=(0, 10), fill="x", anchor="n")  # ลด padding ทั้งหมด
        self._make_login()
    ...
    def _field(self, parent, label_text, show=""):
        # ลดขนาดฟอนต์และช่องไฟให้เตี้ยลง
        tk.Label(parent, text=label_text, bg="#ffffff", fg="#333",
                 font=("Segoe UI", 9, "bold")).pack(anchor="w", padx=28)
        e = tk.Entry(parent, font=("Segoe UI", 11), relief="solid", bd=1, show=show, justify="left")
        e.pack(pady=(2, 8), ipady=5, padx=28, fill="x")
        return e
    ...
    def _make_login(self):
        ...
        tk.Button(self.card, text="SIGN IN", font=("Segoe UI", 11, "bold"),
                  relief="solid", bd=1, command=self.sign_in)\
            .pack(pady=6, ipady=7, padx=28, fill="x")
        tk.Button(self.card, text="CREATE ACCOUNT", font=("Segoe UI", 10),
                  relief="flat", bg="#ffffff", command=self._make_register)\
            .pack(pady=(4, 10))
    ...
    def _make_register(self):
        ...
        self.reg_phone = self._field(self.card, "PHONE")
        self.reg_email = self._field(self.card, "EMAIL")
        self.reg_pass1 = self._field(self.card, "PASSWORD", show="•")
        self.reg_pass2 = self._field(self.card, "CONFIRM PASSWORD", show="•")

        # ปุ่ม: ลดขนาดและช่องไฟให้มองเห็นแน่นอน
        tk.Button(self.card, text="REGISTER", font=("Segoe UI", 11, "bold"),
                  relief="solid", bd=1, command=self.register)\
            .pack(pady=(6, 6), ipady=7, padx=28, fill="x")
        tk.Button(self.card, text="BACK TO LOGIN", font=("Segoe UI", 10),
                  relief="flat", bg="#ffffff", command=self._make_login)\
            .pack(pady=(0, 8))
    ...
    def register(self):
        ...
        if not u or not p1 or not p2:
            messagebox.showerror("Error", "Please fill all required fields."); return
        if p1 != p2:
            messagebox.showerror("Error", "Passwords do not match."); return
        if not valid_username(u):
            messagebox.showerror("Error","Username must contain only A-Z, a-z, 0-9."); return
    ...
    def _make_login(self):
        self.clear_card()
        tk.Label(self.card, text="Sign In", bg="#ffffff", fg="#000000",
                 font=("Segoe UI", 15, "bold")).pack(pady=(12, 4))
        self.login_user = self._field(self.card, "USERNAME")
        self.login_pass = self._field(self.card, "PASSWORD", show="•")
        tk.Button(self.card, text="SIGN IN", font=("Segoe UI", 12, "bold"),
                  relief="solid", bd=1, command=self.sign_in)\
            .pack(pady=6, ipady=8, padx=32, fill="x")
    ...
    def _make_register(self):
        self.clear_card()
        tk.Label(self.card, text="Create Account", bg="#ffffff", fg="#1f2937",
                 font=("Segoe UI", 15, "bold")).pack(pady=(10, 4))
        self.reg_user  = self._field(self.card, "USERNAME")
        self.reg_phone = self._field(self.card, "PHONE")
        self.reg_email = self._field(self.card, "EMAIL")
        self.reg_pass1 = self._field(self.card, "PASSWORD", show="•")
        ...
        tk.Button(self.card, text="REGISTER", font=("Segoe UI", 12, "bold"),
                  relief="solid", bd=1, command=self.register)\
            .pack(pady=(6, 6), ipady=8, padx=32, fill="x")
