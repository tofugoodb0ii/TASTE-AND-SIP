...
class DB:
    ...
    def _migrate(self):
        # เพิ่มคอลัมน์ VAT ใน orders ถ้ายังไม่มี
    ...
    def create_order(self, user_id, cart_items, promo_code, payment_method="SLIP", payment_ref=""):
        ...
        # discount: รองรับ PERCENT_BILL/FLAT_BILL/… (ต่อสินค้าที่ระบุ) เหมือนเดิม
        discount = 0.0
    ...
    def order_detail(self, oid):
        ...
        tops = []  # no toppings in simple mode
        return o, items, tops

    # --- admin CRUD/promos/reports (คงเดิม) ---
...
def create_receipt_pdf(order_id, db: DB, user_row):
    ...
    canv.setFont("Helvetica", 10)
    canv.drawString(x, y, f"Payment: {order['payment_method']}"); y -= 12*mm
    canv.drawString(x, y, "Slip attached in system (see payments.ref).")
