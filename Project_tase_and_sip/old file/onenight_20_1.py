...
# NOTE: ถ้าเป็น path แบบ absolute ให้กำหนดตรง ๆ ด้านล่างแทน
IMG_QR_PATH         = r"C:\Users\thatt\Downloads\qrcode.jpg"   # ใส่ QR ร้านไว้
...
class DB:
    ...
    # --- helpers for AdminView (คงชื่อเดิม)
...
class PaymentDialog(tk.Toplevel):
    def __init__(self, app):
        ...
        # ขยายหน้าต่าง + fixed-size ให้พอดี
        ...
        # ปุ่ม “เสร็จสิ้น” จะ enable หลังอัปโหลด
        ...
        if self.slip_path:
            self._show_slip(self.slip_path)
    ...
    def finish_upload(self):
        ...
        # เปิดปุ่ม Checkout เมื่อกดยืนยันเสร็จสิ้น
...
class App(tk.Tk):
    ...
    def do_search(self):
        ...
        if not kw:
            messagebox.showinfo("Search","กรุณาพิมพ์คำค้นหาเมนู"); return
        self.frames["SearchView"].run_search(kw)
        self.show("Search")
    ...
    def login_ok(self, user_row):
        ...
        # ซ่อนปุ่ม Admin ถ้าไม่ใช่ admin
        if user_row['role']=="admin":
            # ensure visible
            try: self.btn_admin.pack_info()
            except: pass
        else:
            try: self.btn_admin.pack_forget()
            except: pass
        self.show("Shop")
...
class SearchView(ttk.Frame):
    """แสดงผลการค้นหาเมนูจากช่อง Search ด้านบน"""
    def __init__(self, master, app: App):
    ...
    def run_search(self, keyword:str):
        ...
        if not rows:
            ttk.Label(self.holder,text="ไม่พบเมนูที่ค้นหา").pack(anchor="w",pady=8)
            return
        grid=ttk.Frame(self.holder); grid.pack(fill="x")
        col=0; rowframe=None
        for i, p in enumerate(rows):
            if i%4==0:
                rowframe=ttk.Frame(grid); rowframe.pack(fill="x",pady=6)
                col=0
            ProductCard(rowframe, p, self._add).grid(row=0,column=col,padx=6)
            col+=1
