...
def create_receipt_pdf(order_id, db: DB, user_row):
    ...
    c.drawRightString(mx+120*mm, y-6*mm, f"{float(order['discount'] or 0):.2f}")
    ...
    def render_list(self):
        ...
        for title, w in cols:
            ctk.CTkLabel(header, text=title, width=w, anchor="w",
                         ...
                         text_color=PALETTE["bistre"]).pack(side="left", padx=(8 if title=="NAME" else 0), pady=2)
