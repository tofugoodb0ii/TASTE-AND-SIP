...
# - Fancy Login (left image + logo) -> Full App (Shop, Cart, Options by Category, Promo, QR Pay, PDF Receipt)
...
# ---------- Paths: use relative (portable) ----------
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
ASSETS_DIR          = os.path.join(BASE_DIR, "assets")
IMG_DIR             = os.path.join(ASSETS_DIR, "images")
IMG_PRODUCTS_DIR    = os.path.join(IMG_DIR, "products")
IMG_QR_PATH         = os.path.join(IMG_DIR, "qr.png")     # <- ใส่ไฟล์เอง
IMG_AVATARS_DIR     = os.path.join(ASSETS_DIR, "avatars")
REPORTS_DIR         = os.path.join(BASE_DIR, "reports")

# ---------- Login screen images (put your files here) ----------
LEFT_BG_PATH = os.path.join(IMG_DIR, "AVIBRA_1.png")      # BG ซ้าย
LOGO_PATH    = os.path.join(IMG_DIR, "logo.png")          # โลโก้

def ensure_dirs():
    for p in [ASSETS_DIR, IMG_DIR, IMG_PRODUCTS_DIR, IMG_AVATARS_DIR, REPORTS_DIR]:
        os.makedirs(p, exist_ok=True)

# (Optional) simple salt for password hashing
APP_SALT = "change-me-unique"
def sha256(s: str) -> str:
    return hashlib.sha256((APP_SALT + s).encode("utf-8")).hexdigest()
...
class DB:
    ...
    def _seed(self):
        ...
        # >>> Ensure required toppings exist (+10 each)
        need_toppings = [
            # FOOD
            ("ไส้กรอก", 10), ("ไข่ต้ม", 10), ("ไข่เจียว", 10), ("ไข่ดาว", 10), ("ไข่ออนเซ็น", 10),
            # DRINK
            ("วิปครีม", 10), ("ไข่มุก", 10), ("ครีมชีส", 10), ("ซอสช็อกโกแลต", 10),
        ]
        for name, price in need_toppings:
        ...
        # options demo (เดิม: Size/Sweetness) — เก็บไว้ใช้กับเครื่องดื่ม (sweetness)
    ...
    def create_order(self, user_id, channel, pickup_time, cart_items, promo_code, payment_method="QR"):
        ...
        for it in cart_items:
            # ไม่มี size แล้ว แต่เผื่อ future (multiplier = 1 ถ้าไม่มี)
            size_mult = {"S":1.0,"M":1.2,"L":1.5}.get(it.get('size'),1.0)
            base = float(it['base_price']) * size_mult
            tops = sum(float(t['price'])*int(t.get('qty',1)) for t in it.get('toppings',[]))
            line = (base + tops) * int(it['qty'])
            subtotal += line
        ...
        # items
        for it in cart_items:
            # >>> เก็บ options ตามประเภท (ไม่มี size)
            opts = {
                "Protein":   it.get("protein") or None,
                "Spiciness": it.get("spiciness") or None,
                "Sweetness": it.get("sweetness") or None,
            }
            ...
            for tp in it.get('toppings',[]):
                cur.execute("INSERT INTO order_item_toppings(order_item_id,topping_id,qty,price) VALUES(?,?,?,?)",
                            (oi_id, tp.get('id'), tp.get('qty',1), float(tp['price'])))
        # payment row
    ...
    def report_gross_margin(self, start, end):
        # FIX: คำนวณ cost/revenue แยก แล้วสรุปรวม (ไม่ join 1=1)
        return self.conn.execute("""
            WITH sold AS (
            ...
            cogs AS (
                SELECT SUM(s.qty * bl.qty_per_unit * ii.cost_per_unit) AS cost
                FROM sold s
            ...
            SELECT rev.revenue AS revenue,
                   IFNULL(cogs.cost,0) AS cost,
                   (rev.revenue - IFNULL(cogs.cost,0)) AS gross_profit
            FROM rev, cogs
...
def create_receipt_pdf(order_id, db: DB, user_row):
    ...
    for it in items:
        ...
        # แสดงเฉพาะ option ที่มีจริง
        opt_parts = []
        if opts.get("Protein"):   opt_parts.append(f"Protein: {opts.get('Protein')}")
        if opts.get("Spiciness"): opt_parts.append(f"Spicy: {opts.get('Spiciness')}")
        if opts.get("Sweetness"): opt_parts.append(f"Sweetness: {opts.get('Sweetness')}")
        if opt_parts:
            canv.drawString(x+8*mm, y, " / ".join(opt_parts)); y -= 5*mm
        for tp in tps:
...
class ProductDialog(tk.Toplevel):
    """ ปรับตามหมวด: FOOD = โปรตีน/เผ็ด + ท็อปปิ้ง(+10), DRINK = หวาน + ท็อปปิ้ง(+10) """
    TOPPING_PRICE = 10
    FOOD_PROTEINS = ["หมู", "เนื้อ", "กุ้ง", "ปลาหมึก", "ไก่", "เต้าหู้"]
    FOOD_SPICY    = ["ไม่เผ็ด", "เผ็ดน้อย", "เผ็ดกลาง", "เผ็ดมาก"]
    FOOD_TOPS     = ["ไส้กรอก", "ไข่ต้ม", "ไข่เจียว", "ไข่ดาว", "ไข่ออนเซ็น"]
    DRINK_SWEET   = ["0%", "25%", "50%", "75%", "100%"]
    DRINK_TOPS    = ["วิปครีม", "ไข่มุก", "ครีมชีส", "ซอสช็อกโกแลต"]

    def __init__(self, master, db: DB, prod_row, toppings, on_add):
        super().__init__(master)
        self.title(f"Customize - {prod_row['name']}")
        self.db=db; self.prod=prod_row; self.on_add=on_add
        self.geometry("420x560"); self.resizable(False, False)

        # หา category ชื่อ
        cat = db.conn.execute("SELECT name FROM categories WHERE id=?", (prod_row['category_id'],)).fetchone()
        self.category = (cat['name'] if cat else "").upper()

        frm = ttk.Frame(self, padding=10); frm.pack(fill="both", expand=True)

        # เตรียม topping map จาก DB เพื่อหา id ตามชื่อ
        self.topping_rows = {r['name']: r for r in db.toppings()}

        self.var_qty = tk.IntVar(1)
        self.txt_note = tk.Text(frm, height=3)

        # ===== FOOD =====
        if self.category == "FOOD":
            ttk.Label(frm, text="โปรตีน").pack(anchor="w")
            self.var_protein = tk.StringVar(value=self.FOOD_PROTEINS[0])
            ttk.Combobox(frm, values=self.FOOD_PROTEINS, textvariable=self.var_protein,
                         state="readonly").pack(fill="x", pady=4)

            ttk.Label(frm, text="ความเผ็ด").pack(anchor="w")
            self.var_spicy = tk.StringVar(value=self.FOOD_SPICY[0])
            ttk.Combobox(frm, values=self.FOOD_SPICY, textvariable=self.var_spicy,
                         state="readonly").pack(fill="x", pady=4)

            ttk.Label(frm, text="ท็อปปิ้ง (+10)").pack(anchor="w", pady=(8,0))
            self.top_vars = {}
            frt = ttk.Frame(frm); frt.pack(fill="x")
            for name in self.FOOD_TOPS:
                v = tk.IntVar(0)
                ttk.Checkbutton(frt, text=name, variable=v).pack(anchor="w")
                self.top_vars[name] = v

        # ===== DRINK =====
        elif self.category == "DRINK":
            ttk.Label(frm, text="ความหวาน").pack(anchor="w")
            self.var_sweet = tk.StringVar(value=self.DRINK_SWEET[2])
            ttk.Combobox(frm, values=self.DRINK_SWEET, textvariable=self.var_sweet,
                         state="readonly").pack(fill="x", pady=4)

            ttk.Label(frm, text="ท็อปปิ้ง (+10)").pack(anchor="w", pady=(8,0))
            self.top_vars = {}
            frt = ttk.Frame(frm); frt.pack(fill="x")
            for name in self.DRINK_TOPS:
                v = tk.IntVar(0)
                ttk.Checkbutton(frt, text=name, variable=v).pack(anchor="w")
                self.top_vars[name] = v

        # ===== OTHER (เช่น DESSERT) — ไม่มีตัวเลือกพิเศษ =====
        else:
            self.top_vars = {}

        frq = ttk.Frame(frm); frq.pack(fill="x", pady=6)
        ttk.Label(frq, text="Qty").grid(row=0,column=0,sticky="w")
        ttk.Spinbox(frq, from_=1, to=99, textvariable=self.var_qty, width=6).grid(row=0,column=1,padx=6)
        ...
        # เพิ่มความเสถียรของ dialog
        self.transient(master)
        self.grab_set()
        self.focus()

    def _selected_toppings(self):
        tops=[]
        for name, v in self.top_vars.items():
            if v.get()==1:
                trow = self.topping_rows.get(name)
                tops.append({
                    "id": trow['id'] if trow else None,
                    "name": name,
                    "price": float(self.TOPPING_PRICE),
                    "qty": 1
                })
        return tops
    ...
    def _add(self):
        ...
        # ใส่ option ตามประเภท
        if self.category == "FOOD":
            item["protein"]   = self.var_protein.get()
            item["spiciness"] = self.var_spicy.get()
        elif self.category == "DRINK":
            item["sweetness"] = self.var_sweet.get()

        item["toppings"] = self._selected_toppings()

        try:
            self.on_add(item)    # เพิ่มเข้าตะกร้า
        finally:
            self.destroy()
...
class LoginView(ttk.Frame):
    """ Login view (เรียบในแอปหลัก สำหรับ fallback/แอดมิน) """
...
class ShopView(ttk.Frame):
    ...
    def customize(self, prod_row):
        # ส่ง toppings ของ DB ให้ dialog เพื่อแมพชื่อ -> id
    ...
    def _totals(self):
        ...
        for it in self.app.cart:
            size_mult={"S":1.0,"M":1.2,"L":1.5}.get(it.get("size"),1.0)  # ไม่มี size → 1.0
            base=float(it['base_price'])*size_mult
            tops=sum(float(t['price'])*int(t.get('qty',1)) for t in it.get('toppings',[]))
            subtotal+=(base+tops)*int(it['qty'])
        discount=0.0
    ...
    def refresh(self):
        ...
        for it in self.app.cart:
            ...
            tops=sum(float(t['price'])*int(t.get('qty',1)) for t in it.get('toppings',[]))
            line=(base+tops)*int(it['qty'])
            # สร้างคำอธิบาย Option สวยๆ
            parts=[]
            if it.get("protein"):   parts.append(f"โปรตีน:{it['protein']}")
            if it.get("spiciness"): parts.append(f"เผ็ด:{it['spiciness']}")
            if it.get("sweetness"): parts.append(f"หวาน:{it['sweetness']}")
            desc = " / ".join(parts) if parts else "-"
            self.tv.insert("", "end", values=(f"{it['name']} [{desc}]", it['qty'], f"{line:.2f}"))
...
class AdminView(ttk.Frame):
    ...
    def _adjust(self, sign):
        ...
        try:
            self.app.db.adjust_inventory(inv_id, sign*abs(float(val))); self.reload_inventory()
        except Exception as e:
            messagebox.showerror("Adjust", f"Invalid number: {e}")
...
class ProductEditor(tk.Toplevel):
    ...
    def save(self):
        ...
        try:
            price=float(self.ep.get().strip())
        except ValueError:
            messagebox.showerror("Error","Base Price must be a number."); return
        img=self.ei.get().strip()
        try:
            act=int(self.ea.get().strip() or 1)
        except ValueError:
            messagebox.showerror("Error","Active must be 1 or 0."); return
...
class PromoEditor(tk.Toplevel):
    ...
    def save(self):
        ...
        try:
            ...
            messagebox.showerror("Error","Value/Min Spend must be numbers."); return
...
class FancyLogin(tk.Tk):
    def __init__(self):
        ...
        self.bg_img = _tk_load_and_cover(LEFT_BG_PATH, LEFT_W, LEFT_H)
        canvas = tk.Canvas(left, width=LEFT_W, height=LEFT_H, highlightthickness=0, bg="#ececec")
        canvas.pack(fill="both", expand=True)
        if self.bg_img: canvas.create_image(LEFT_W // 2, LEFT_H // 2, image=self.bg_img)
        canvas.create_text(32, 36, text="WELCOME TO \nTASTE AND SIP", fill="white", font=("Georgia", 32, "bold"), anchor="nw")
        canvas.create_text(36, 84, text="\n\nFOOD AND DRINK !", fill="white", font=("Georgia", 16), anchor="nw")

        right = tk.Frame(self, bg="#f8eedb")
        right.pack(side="left", fill="both", expand=True)
        self.logo_img = _tk_load_and_fit(LOGO_PATH, 220, 220)
        logo_wrap = tk.Frame(right, bg="#f5efe4"); logo_wrap.pack(pady=20)
        if self.logo_img:
            tk.Label(logo_wrap, image=self.logo_img, bg="#f8eedb").pack()
        else:
            tk.Label(logo_wrap, text="TASTE AND SIP", bg="#f5efe4", font=("Segoe UI", 22, "bold")).pack()
    ...
    def _field(self, parent, label_text, show=""):
        tk.Label(parent, text=label_text, bg="#ffffff", fg="#333", font=("Segoe UI", 10, "bold")).pack(anchor="w", padx=40)
        e = tk.Entry(parent, font=("Segoe UI", 12), relief="solid", bd=1, show=show, justify="left")
    ...
    def _make_login(self):
        self.clear_card()
        tk.Label(self.card, text="Sign In", bg="#ffffff", fg="#000000", font=("Segoe UI", 16, "bold")).pack(pady=(22,6))
        self.login_user = self._field(self.card, "USERNAME")
        self.login_pass = self._field(self.card, "PASSWORD", show="•")
        tk.Button(self.card, text="SIGN IN", font=("Segoe UI", 12, "bold"), relief="solid", bd=1, command=self.sign_in)\
            .pack(pady=6, ipady=8, padx=40, fill="x")
        tk.Button(self.card, text="CREATE ACCOUNT", font=("Segoe UI", 11), relief="flat", bg="#ffffff",
                  command=self._make_register).pack(pady=(8,20))

    def _make_register(self):
        self.clear_card()
        tk.Label(self.card, text="Create Account", bg="#ffffff", fg="#1f2937", font=("Segoe UI", 16, "bold")).pack(pady=(22,6))
        ...
        tk.Button(self.card, text="REGISTER", font=("Segoe UI", 12, "bold"), relief="solid", bd=1, command=self.register)\
            .pack(pady=6, ipady=8, padx=40, fill="x")
        tk.Button(self.card, text="BACK TO LOGIN", font=("Segoe UI", 11), relief="flat", bg="#ffffff",
    ...
    def sign_in(self):
        ...
        if row:
            ...
            self.destroy()  # ปิดหน้าล็อกอิน แล้วเปิดแอปหลัก
            app = App()
            app.login_ok(row)  # ตั้งค่า user และเข้า Shop
            app.mainloop()
        else:
