...
IMG_QR_PATH      = os.path.join(IMG_DIR, r"C:\Users\thatt\Downloads\qrcode.jpg")  # วาง qrcode.jpg ใน assets/images/
...
class AuthApp(ctk.CTk):
    ...
    def _draw_left_bg(self, *_):
        ...
        ctk.CTkLabel(self.left,
                     text=f"WELCOME TO\n{APP_TITLE} \nwe've food drink and dessert".upper(),
                     font=ctk.CTkFont(size=28, weight="bold"),
                     text_color="white",
                     fg_color="transparent"  # ทำให้ลอยบนภาพจริง ๆ
                     ).place(x=28, y=28)
...
class ProfilePage(ctk.CTkFrame):
    ...
    def __init__(self, app: CTkMain):
        ...
        ctk.CTkLabel(self, text="PROFILE", font=ctk.CTkFont(size=24, weight="bold"),
                     text_color=PALETTE["bistre"], fg_color="transparent").pack(anchor="w", padx=20, pady=(10,4))
        ...
        def row(r, label):
            ctk.CTkLabel(form, text=label, text_color=PALETTE["bistre"], fg_color="transparent").grid(row=r, column=0, sticky="e", padx=6, pady=8)
    ...
    def change_avatar(self):
        ...
        try:
            ...
            row = self.app.db.conn.execute("SELECT * FROM users WHERE id=?", (self.app.user["id"],)).fetchone()
            self.app.user = dict(row)
            self.app.refresh_topbar_user()
            self.on_show()
        except Exception as e:
    ...
    def save(self):
        ...
        # อยู่หน้าเดิมและแสดงค่าปัจจุบัน
        self.on_show()

class CategoryPage(ctk.CTkFrame):
    """Grid การ์ดสินค้า + Search"""
    def __init__(self, app: CTkMain):
        super().__init__(app.body, fg_color=PALETTE["antique"]); self.app=app
    ...
    def render(self):
        ...
        for p in products:
            ...
            ctk.CTkLabel(card, image=img, text=("No Image" if not img else ""), fg_color="transparent").pack(pady=(10,6))
            ctk.CTkLabel(card, text=p["name"], font=ctk.CTkFont(size=14, weight="bold"),
                         text_color=PALETTE["bistre"], fg_color="transparent").pack()
...
class CartPage(ctk.CTkFrame):
    """ตะกร้า + QR + Upload Slip + Checkout"""
    def __init__(self, app: CTkMain):
        super().__init__(app.body, fg_color=PALETTE["antique"]); self.app=app
        self.slip_path = None
        ...
        ctk.CTkLabel(pay, text="QR CODE", text_color=PALETTE["bistre"], fg_color="transparent").pack(pady=(10,4))
        if os.path.exists(self.app.qr_path):
            img = ctk_img(self.app.qr_path, (210,210))
            ctk.CTkLabel(pay, image=img, text="", fg_color="transparent").pack()
        else:
            ctk.CTkLabel(pay, text="(put qrcode.jpg in assets/images)", fg_color="transparent").pack()

        ctk.CTkLabel(pay, text="UPLOAD SLIP", text_color=PALETTE["bistre"], fg_color="transparent").pack(pady=(12,6))
        ...
        self.lbl_sub = ctk.CTkLabel(pay, text="", fg_color="transparent")
        self.lbl_dis = ctk.CTkLabel(pay, text="", fg_color="transparent")
        self.lbl_vat = ctk.CTkLabel(pay, text="", fg_color="transparent")
        self.lbl_tot = ctk.CTkLabel(pay, text="", font=ctk.CTkFont(size=16, weight="bold"), fg_color="transparent")
    ...
    def render_list(self):
        ...
        if not self.app.cart:
            ctk.CTkLabel(self.listf, text="Cart is empty.", text_color=PALETTE["bistre"], fg_color="transparent").pack(pady=12)
        for it in self.app.cart:
            row = ctk.CTkFrame(self.listf, fg_color=PALETTE["white"], corner_radius=12,
                               border_color=PALETTE["pale_taupe"], border_width=1)
            row.pack(fill="x", pady=4)
            ctk.CTkLabel(row, text=it["name"], width=260, anchor="w", text_color=PALETTE["bistre"], fg_color="transparent").pack(side="left", padx=8, pady=8)
            ctk.CTkLabel(row, text=f"x{it['qty']}", width=60, fg_color="transparent").pack(side="left")
            ctk.CTkLabel(row, text=f"{it['base_price']*it['qty']:.2f} ฿", width=100, fg_color="transparent").pack(side="left")
...
class AdminReports(ctk.CTkFrame):
    ...
    def _fill(self, headers, rows):
        ...
        for h, w in zip(headers, widths):
            ctk.CTkLabel(head, text=h, width=w, anchor="w", text_color=PALETTE["bistre"], fg_color="transparent").pack(side="left", padx=6, pady=4)
        for r in rows:
            row = ctk.CTkFrame(self.out, fg_color=PALETTE["white"], corner_radius=10,
                               border_color=PALETTE["pale_taupe"], border_width=1)
            row.pack(fill="x", pady=4)
            for val, w in zip(r, widths):
                ctk.CTkLabel(row, text=str(val), width=w, anchor="w", text_color=PALETTE["bistre"], fg_color="transparent").pack(side="left", padx=6, pady=6)
    def run_daily(self):
        s,e=self._range()
