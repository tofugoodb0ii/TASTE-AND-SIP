...
# - Login/Sign up/Forgot Password: customtkinter ตามดีไซน์ (ภาพ cover ซ้าย + การ์ดครีมโค้งมนขวา)
# - หลังล็อกอินสำเร็จ เปิดแอปหลัก Tk (Shop, Cart, Promotions, QR, PDF Receipt, Admin: Products/Inventory/Promos/Reports)
...
# ---- customtkinter สำหรับหน้า Login ----
...
class App(tk.Tk):
    ...
    def logout(self):
        ...
        # ปิดหน้าหลัก แล้วกลับไปหน้า CTk Login เดิม (เหมือนตอนเปิดครั้งแรก)
        if hasattr(self, "_back_to_ctk_login") and callable(self._back_to_ctk_login):
            ...
            try:
                ...
                launch_ctk_login(existing_db=getattr(self, "db", None))

# ---------- Views (fallback/simple inside main app) ----------
class LoginView(ttk.Frame):
    """fallback login (ยังคงไว้ แต่ปกติจะเข้าแอปผ่าน CTk Login ก่อน)"""
...
# ======================= CTk Login (ตามดีไซน์ของคุณ) =======================
RIGHT_BG   = "#f8eedb"   # cream tone
...
def launch_ctk_login(existing_db=None):
    """เปิดหน้า CTkLogin ใหม่ (ใช้ connection เดิมถ้ามี)"""
    ensure_dirs()
    login = CTkLogin(db_path=DB_FILE, left_bg_path=LEFT_BG_PATH, logo_path=LOGO_PATH)
    if existing_db is not None:
        login.db = existing_db
    login.mainloop()

class CTkLogin(ctk.CTk):
    """หน้าล็อกอิน/สมัคร/ลืมรหัส ผ่าน customtkinter; ใช้ DB เดียวกับแอปหลัก"""
    def __init__(self, db_path: str = DB_FILE, left_bg_path: str = LEFT_BG_PATH, logo_path: str = LOGO_PATH):
        ...
        self.db = DB(db_path)   # ใช้คลาส DB เดิม
        ...
        self.show_signin()
        self._verified_username = None  # สำหรับ forgot password
    ...
    def _signin(self):
        ...
        if row:
            # ปิด CTk แล้วเปิดแอปหลัก Tk; ผูก callback สำหรับกลับหน้า CTk Login เมื่อ Logout
            self.destroy()
            app = App()
            app.db = self.db  # reuse connection
            app.login_ok(row)
...
if __name__ == "__main__":
    ensure_dirs()
    # เริ่มจาก CTk Login (ดีไซน์ใหม่) และรองรับกลับมาหน้านี้เมื่อกด Logout
    launch_ctk_login()
